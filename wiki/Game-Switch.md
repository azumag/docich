...
## draining（試合境界待ち）と Soren 側の受け口

`require_round_boundary = true` のゲームへの切替は、旧ゲームを **draining** フェーズで
試合完走まで待つ。writer lock は解放され、agent は入力を継続する。CLI ゲーム
（Robots 等）の自然終了プロンプト（`Another game?`）のみを境界とし、`Really quit?`
は誤認しない（観測のみ・timeout で fail-closed・強制終了なし）。スコアは ack 前に
`round_boundary_result.json` へ原子保存する。

境界を ack した後の**資源停止は Soren 側の二相ハンドオーバー**で行う: boundary では
次ゲームだけを保留し、coordinator の明示 stop 制御後に共有表示 readiness gate を通って
から不可逆な Unity Quit へ進む。詳細は [[ゲームライフサイクル|Game-Lifecycle]]。
