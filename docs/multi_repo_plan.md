...
4. **docichcc フィルタの二重管理解消**: → **解消中 (2026-08-20)**。soviet_now の互換コピー (`native/ffmpeg/`, `lib/closed_captions.py`) は、docich のバージョン付きビルドアーティファクト (`native/ffmpeg/build.sh` → `MANIFEST.json`、ビルドルート `docich-cc-<sha>` 規約) の消費へ置換する。進行: Phase A (docich 側: MANIFEST 出力 + ドキュメント更新) → Phase B (soviet_now 側: `closed_captions.py` の docich CLI 委譲シム化 + `native/ffmpeg` 削除、ユーザー合意ゲート) → Phase C (VM 本番: ビルド + `.env` 切替 + 実測)。
