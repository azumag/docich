...
ターン制で「保留＝無入力」を続けても局面は変わりません。安全な移動候補があるのにゼロ行動を返す問題と、
文脈不明なので入力を止める必要がある局面を区別します。全局面の無停止・生存・昇天は保証しません。
...
| 隣接creatureなし、低HP/移動障害の保留、Hungryなし | `.` 1ターン。隣接 `f` も休止を禁止 |
| 隣接creatureあり、移動障害なし | 可視安全マスへの縦横1歩 → 斜め1歩 → creature への通常接触1回 |
| `Blind/Conf/Stun/Hallu` と隣接creature | 無入力。方向の安全性・ペット保護を信用しない |
| `Hungry`（critical HPとの同時発生も含む） | 可視探索/退避 → 通常接触 → hold。generic restは禁止 |
| 通常の探索 | 可視 `.` / `#` / `<` / `>` への縦横優先の1歩。縦横候補なしなら斜め |
| 安全マスなし・隣接creatureなしの探索保留 | 既存の `.` 1ターンを維持 |
...
`Really`、`Would/Could/Should/Do/Did/Can/Will/Are you` で始まる質問の断片を根拠に保留します。
これらは質問としての保留条件であり、一般的な単語折り返し検出ではありません。
...
- 通常接触後の攻撃確認は即拒否し、原因となった方向を除外します。別方向の退避/接触があれば次に試します。
  全候補を拒否されたら `progress_blocked` にし、同じ相手へ接触→拒否を永久に繰り返しません。
- 除外は現在の地図・位置・階層に限定し、これらが変われば解除します。メッセージや `T` の変化だけでは解除しません。
  別brain/runtimeには引き継ぎません。地図が変わると同じpeacefulへ再度接触する可能性はあります。
- 戦闘は位置不変でも `T` が進めば継続可能です。`T` なし・完全同一フレームでは進行を証明できないため、
  有限回後に `progress_blocked` にします。実際は進んだ戦闘を保守的に止める可能性があります。
