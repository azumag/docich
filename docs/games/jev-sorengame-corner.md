...
`recover` / `diagnose` / `recover-diagnose` / `refresh-bridge` / `recover-bridge` の固定操作だけをdispatchする。workflowはproductionが現在のprotected
...
`recover-diagnose` はこの復旧条件を固定カテゴリの終了コードだけで確認する読み取り専用操作である。
