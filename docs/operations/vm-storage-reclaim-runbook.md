...
| 2026-09-23 | Phase 2c apply（**AivisSpeech opt-in 削除**、PR #1005 実装の `aivis_engine=true`） | owner 確認「再取得可能」・Astra 実行（dry-run run 35789915210 / apply run **35789981809**、いずれも `aivis_engine=1`・exit 0） | 80% / avail 9,855,180,800 B（status run 35789862552, `configured @ 8b2c97a1`） | **77% / avail 11,160,809,472 B（+1,305,628,672 B ≒ +1.22GB）**（status run 35790046805） | 対象実測（read-only SSH）: **`AivisSpeech-Engine` 1.11GB gone OK** / **VOICEVOX engine binary OK + 稼働中プロセス確認**（`/opt/voicevox/current/run --host 127.0.0.1 --port 50021`）/ **TTS 経路動作実測**: `.say_queue` に5分以内の新規3件（playlist concat 含む）＝削除で読み上げが止まっていない / gate KEEP 継続: `s91test`・`soren-src` / protected 3件＋**配信 ffmpeg（pid 2250958, 18:50 稼働）** 生存 / diagnostics（run 35790152742）: `game_switch` ready、audio_worker/chat_worker/soren_loop 全 alive。**事故と対応**: ①直後の並行セッション `configure_jev` run（35790001942, failure, confirm 欠落）を自前 apply と取り違えて報告した → run log の `INPUT_OPERATION` で誤りを自己発見、自前は 35789981809 exit 0 で確定。②main push `988a895d`（#1008）の deploy run 35789988996 が **cancelled のまま main↔VM が1コミット乖離**（`cancel-in-progress: false` なので自動キャンセルではなく明示キャンセル）→ rerun で success、status run 35790460821 で **`configured @ 988a895d` = origin/main 完全一致**を確認して同期復元。最終: 77% / avail 11,129,397,248 B。 |
