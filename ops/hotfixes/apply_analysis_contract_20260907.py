...
SOURCE_SHA='074ff24f18b3409141c9d0ef83457a5edc84c60a'
...
def manifest():
    return json.loads(Path(__file__).with_name('analysis_contract_20260907.json').read_text())
...
def validate_payload(doc,payload):
    ...
    for rel,v in doc['files'].items():
        if policy.digest(payload[rel])!=v['new'] or v['mode']!=MODES[rel]:
            raise ValueError('source hash/mode mismatch')
    compile(payload[PATHS[0]].decode(),PATHS[0],'exec')
    payload[PATHS[1]].decode('utf-8')
    subprocess.run(['bash','-n'],input=payload[PATHS[2]],check=True,timeout=15)
