...
`check` は静的な前提のみ確認し、モデルを呼びません。実効shadowの確認はサービス内で設定を読み込んだ後、worker開始前に行います。enforceなら拒否し、shadowをreadonly/exportしてからworker snapshotをsourceします。worker自身も既存設定を読みますがreadonlyなmodeは上書きできません。設定読込失敗時は起動拒否です。
