...
class OwnershipBoundaryTests(unittest.TestCase):
    def test_managed_keys_never_overlap_with_678s_own_script(self):
        ...
        self.assertEqual(set(route.MANAGED_KEYS) & set(classifier.MANAGED_KEYS), set())

    def test_managed_keys_are_exactly_the_docich_canonical_route_keys(self):
        self.assertEqual(set(route.MANAGED_KEYS),
                         {"DOCICH_SEMANTIC_BACKEND", "DOCICH_JEV_ROUTE", "DOCICH_JEV_VERCEL_API_KEY"})


class ConfigureDirectEnvTests(unittest.TestCase):
    def test_sets_backend_and_route_without_touching_678_keys(self):
    ...
    def test_replaces_only_its_own_prior_assignment_not_duplicated(self):
        with tempfile.TemporaryDirectory(prefix="jev-route-") as raw:
            ...
            current = env_file.read_text(encoding="utf-8")
            self.assertEqual(current.count("DOCICH_SEMANTIC_BACKEND="), 1)
            self.assertEqual(current, "DOCICH_SEMANTIC_BACKEND=jev\nDOCICH_JEV_ROUTE=direct\n")
...
class DisableEnvTests(unittest.TestCase):
    def test_disable_clears_backend_and_removes_route_and_vercel_key(self):
        with tempfile.TemporaryDirectory(prefix="jev-route-") as raw:
            env_file = Path(raw) / ".env"
            env_file.write_text(
                "KEEP=value\n"
...
class VerifyJevRouteTests(unittest.TestCase):
    def test_disabled_rejects_a_worker_still_reporting_jev(self):
        verify = route.verify_jev_route(None)
        verify({})  # no error
        verify({"DOCICH_SEMANTIC_BACKEND": ""})  # no error
        with self.assertRaises(route.ConfigureError):
            verify({"DOCICH_SEMANTIC_BACKEND": "jev"})

    def test_direct_requires_backend_route_and_typesafe_key(self):
        verify = route.verify_jev_route("direct")
        verify({"DOCICH_SEMANTIC_BACKEND": "jev", "DOCICH_JEV_ROUTE": "direct",
                "TYPESAFE_API_KEY": "present"})
        with self.assertRaises(route.ConfigureError):
            verify({"DOCICH_SEMANTIC_BACKEND": "legacy", "DOCICH_JEV_ROUTE": "direct",
                    "TYPESAFE_API_KEY": "present"})
        with self.assertRaises(route.ConfigureError):
            verify({"DOCICH_SEMANTIC_BACKEND": "jev", "DOCICH_JEV_ROUTE": "vercel",
                    "TYPESAFE_API_KEY": "present"})
        with self.assertRaises(route.ConfigureError):
            verify({"DOCICH_SEMANTIC_BACKEND": "jev", "DOCICH_JEV_ROUTE": "direct"})
    ...
    def test_vercel_requires_its_own_key_not_typesafe_api_key(self):
        ...
        verify({"DOCICH_SEMANTIC_BACKEND": "jev", "DOCICH_JEV_ROUTE": "vercel",
                "DOCICH_JEV_VERCEL_API_KEY": "present"})
        with self.assertRaises(route.ConfigureError):
            verify({"DOCICH_SEMANTIC_BACKEND": "jev", "DOCICH_JEV_ROUTE": "vercel",
                    "TYPESAFE_API_KEY": "present"})
