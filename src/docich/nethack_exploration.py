...
It issues at most one cardinal movement key per observation.
...
class NethackExplorer:
    ...
    def _neighbors(self, obs: NethackObservation, pos: tuple[int, int]):
        for dx, dy, key in CARDINAL:
            nxt = (pos[0] + dx, pos[1] + dy)
            if _passable(obs, nxt):
                yield nxt, key
    ...
    def plan_step(self, obs: NethackObservation) -> ExplorationStep | None:
        ...
        while queue:
            ...
            for nxt, key in self._neighbors(obs, pos):
                if nxt in parent:
                    continue
                parent[nxt] = (pos, key)
        ...
        if candidates:
            ...
            direct = [nxt for nxt, _key in self._neighbors(obs, start)]
            if not direct:
                return None
            target = min(
                direct,
        ...
        # Walk backward to obtain the first cardinal key on the BFS path.
        cursor = target
        ...
        while cursor != start:
            ...
            if edge is None:
                ...
                dx = cursor[0] - start[0]
                dy = cursor[1] - start[1]
                match = next((key for ex, ey, key in CARDINAL if (ex, ey) == (dx, dy)), None)
                first_key = match
                first_target = cursor
                break
            prev, key = edge
