...
def bot_default_weights(game: str) -> dict:
    """各 bot brain の既定重み (brain スクリプトの DEFAULT_WEIGHTS と単一ソース)。"""
    import importlib.util

    presets = _bot_presets()
    if game not in presets:
        raise ValueError(f"bot preset がありません: {game}")
    script = Path(__file__).resolve().parents[3] / presets[game]["bot_cmd"][1]
    spec = importlib.util.spec_from_file_location(f"_docich_brain_{game}", script)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return dict(module.DEFAULT_WEIGHTS)
