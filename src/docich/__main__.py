...
def _jev_corner_argv(argv: list[str]) -> list[str] | None:
    if argv and argv[0] == "jev-corner":
        return argv[1:]
    if len(argv) >= 3 and argv[0] == "--config" and argv[2] == "jev-corner":
        return ["--config", argv[1], *argv[3:]]
    if len(argv) >= 2 and argv[0].startswith("--config=") and argv[1] == "jev-corner":
        return [argv[0], *argv[2:]]
    return None


jev_corner_argv = _jev_corner_argv(sys.argv[1:])
if jev_corner_argv is not None:
    from .jev_corner import main as jev_corner_main

    sys.exit(jev_corner_main(jev_corner_argv))
