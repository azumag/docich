...
def _offline_gate(
    ...
    suite = _read_json_bounded(regression_root / "suite.json")
...
def _critical_tags(obs: NethackObservation, decision: PolicyDecision) -> tuple[str, ...]:
    ...
    if decision.intent in {"assess_contact", "survival_emergency", "status_emergency", "food_emergency"}:
    ...
    def _evaluate_job(self, job: _ShadowJob) -> None:
        ...
        if result.status == "proposed" and result.proposal is not None:
            evaluation = evaluate_proposal(
                ...
                current_inventory=job.inventory,  # type: ignore[arg-type]
            )
