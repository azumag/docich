...
class RetroArchAdapter(Adapter):
    ...
    # --- [retroarch] table helpers -----------------------------------

    def _retroarch_raw(self) -> dict:
        raw = self.ctx.game.raw.get("retroarch", {})
        if not isinstance(raw, dict):
            raise AdapterError("[retroarch] はテーブルである必要があります")
        return raw

    def _resolved_buttons(self) -> dict[str, tuple[str, str]]:
        pad_map = self._retroarch_raw().get("pad_map", {}) or {}
    ...
    def _resolve_rom(self) -> Path:
        rom = self._retroarch_raw().get("rom")
        if not rom:
            raise AdapterError(f"[retroarch] に rom が設定されていません ({self.ctx.game.path})")
        p = Path(rom)
        if not p.is_absolute():
            p = self.ctx.g.repo_root / p
        if not p.is_file():
            raise AdapterError(
    ...
    def _resolve_core(self) -> str:
        core = self._retroarch_raw().get("core", "auto") or "auto"
        if core != "auto":
            p = Path(core)
            if not p.is_file():
    ...
    def prepare(self) -> None:
        ...
        rdir = self.ctx.state.retroarch_dir
        ...
        audio_enable = "true" if self.ctx.g.audio.enabled else "false"
        ...
        lines = [
            ...
            f'network_cmd_port = "{NETWORK_CMD_PORT}"',
