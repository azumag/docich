...
class TestShadowEvaluation(unittest.TestCase):
    def test_perfect_evidence_can_only_become_eligible_for_review(self) -> None:
        ...
        self.assertTrue(report.evidence_integrity_passed)
    ...
    def test_unattributed_producer_failure_blocks_every_source(self) -> None:
        criteria = ReadinessCriteria(
            min_comparable=1,
            min_observation_match_rate=1.0,
            min_map_cell_match_rate=1.0,
        ...
        producer_failure = json.dumps(
            {
                "schema_version": 1,
                "status": "invalid",
                "source": "<unknown>",
                "evidence_kind": "producer_error",
                "policy_effect": "none",
            }
        )
        report = evaluate_jsonl_lines(
            [event("match", source="a"), event("match", source="b"), producer_failure],
            criteria=criteria,
        )
        self.assertFalse(report.evidence_integrity_passed)
        self.assertEqual(report.unattributed_invalid_events, 1)
        self.assertEqual({item.source for item in report.sources}, {"a", "b"})
        for metrics in report.sources:
            self.assertFalse(metrics.eligible_for_review)
            self.assertTrue(
                any("unattributed" in reason for reason in metrics.readiness_reasons)
            )

    def test_malformed_evidence_blocks_otherwise_eligible_source(self) -> None:
        criteria = ReadinessCriteria(
            min_comparable=1,
            min_observation_match_rate=1.0,
            min_map_cell_match_rate=1.0,
        ...
        report = evaluate_jsonl_lines(
            [event("match", source="a"), "not-json"],
            criteria=criteria,
        )
        self.assertFalse(report.evidence_integrity_passed)
        self.assertEqual(report.malformed_lines, 1)
        self.assertFalse(report.sources[0].eligible_for_review)
        self.assertTrue(
            any("malformed" in reason for reason in report.sources[0].readiness_reasons)
        )

    def test_unknown_event_status_blocks_otherwise_eligible_source(self) -> None:
        criteria = ReadinessCriteria(
            min_comparable=1,
            min_observation_match_rate=1.0,
            min_map_cell_match_rate=1.0,
        ...
        report = evaluate_jsonl_lines(
            [
                event("match", source="a"),
                json.dumps({"schema_version": 1, "status": "future-status", "source": "a"}),
            ],
            criteria=criteria,
        )
        self.assertFalse(report.evidence_integrity_passed)
