...
def candidate(now, symbol="7203", *, momentum="20", volume="2", turnover="50000000",
    ...
    return Candidate(symbol=symbol, ts=now, price="1000", turnover_jpy=turnover,
...
class SelectorTests(unittest.TestCase):
    def setUp(self):
        ...
        self.now = moment()
        self.policy = SelectorPolicy(focused_universe_count=2, candidate_count=5,
                                     replace_cooldown_s=15, replace_margin="0.20", warmup_s=60)
    ...
    def test_hysteresis_and_cooldown_prevent_ranking_churn(self):
        ...
        first = select_universe(initial, self.policy, now=self.now, current_symbols=[], held_symbols=[],
                                last_replaced_at=0, session_start=self.now-120)
        self.assertEqual(first["focused_symbols"], ["7203", "9984"])
        small_move = [candidate(self.now+5, "7203", momentum="75"),
                      candidate(self.now+5, "9984", momentum="70"),
                      candidate(self.now+5, "6758", momentum="78")]
        cooled = select_universe(small_move, self.policy, now=self.now+5,
                                 current_symbols=first["focused_symbols"], held_symbols=[],
                                 last_replaced_at=first["last_replaced_at"], session_start=self.now-120)
        self.assertEqual(cooled["focused_symbols"], first["focused_symbols"])
        after_cooldown = select_universe(small_move, self.policy, now=self.now+30,
                                         current_symbols=first["focused_symbols"], held_symbols=[],
                                         last_replaced_at=first["last_replaced_at"], session_start=self.now-120)
        self.assertEqual(after_cooldown["focused_symbols"], first["focused_symbols"])
        breakout = [candidate(self.now+31, "7203", momentum="75"),
                    candidate(self.now+31, "9984", momentum="50"),
                    candidate(self.now+31, "6758", momentum="200")]
        replaced_result = select_universe(breakout, self.policy, now=self.now+31,
                                          current_symbols=first["focused_symbols"], held_symbols=[],
                                          last_replaced_at=first["last_replaced_at"], session_start=self.now-120)
        self.assertIn("6758", replaced_result["focused_symbols"])
        self.assertTrue(replaced_result["changed"])

    def test_held_symbol_is_pinned_even_if_scanner_drops_it(self):
        result = select_universe([candidate(self.now, "9984", momentum="100")], self.policy,
                                 now=self.now, current_symbols=[], held_symbols=["7203"],
                                 last_replaced_at=0, session_start=self.now-120)
    ...
    def test_warmup_blocks_entries_without_blocking_selection(self):
        result = select_universe([candidate(self.now, "7203")], self.policy,
                                 now=self.now, current_symbols=[], held_symbols=[],
                                 last_replaced_at=0, session_start=self.now-30)
    ...
    def test_file_candidate_feed_is_realtime_and_fresh_only(self):
        ...
        write_json(path, {"market": "stocks", "realtime": True, "as_of": self.now,
...
class RuntimeSelectorTests(unittest.TestCase):
    ...
    def test_dynamic_selection_supplies_quote_symbols_and_respects_warmup(self):
        calls = []
        ...
        def quotes(config, _market, _root, _now):
            ...
            return []
        self.lease(self.now)
        begin, _ = report_window("stocks", self.now)
        ...
        self.assertGreaterEqual(self.now - begin, 60)

    def test_entry_cutoff_accounts_for_max_hold_and_exit_buffer(self):
        policy = Policy(max_hold_s=900)
        write_json(self.runtime.root / "active-policy.json", {"policy": asdict(policy)})
        def candidates(_config, _root, now, _policy):
