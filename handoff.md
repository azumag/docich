...
- **実再生確認**: 03:29:23に非Global Voicesの「日米外務次官協議 中国や北朝鮮対応で緊密な連携を確認」を1051字でキュー投入。本文に出典行なし、`.cc_text`なし。13チャンクの字幕同期bundleを作り、03:30:35に実再生開始。`.playing`本文とbundle内字幕をgrepし、`出典` / Google News / TBS NEWS / NHK / Global Voicesはいずれも0件、チャット出典sidecarも不在。**視聴者向け実経路で無出典を確認済み**。ニュース再生後、HUPが長い処理待ちで反映されなかったaudio workerだけを正規TERMし、03:36:48に新PID `2766170` で起動（進行中の時事ニュース事前合成は完了済みで損失なし）。以後は再生直前ガードも起動時から読込済み。Global Voices側の実再生は未発生だが、VM回帰で音声・生成プロンプト・字幕/チャットに出典が残ることを確認済み。
...
- **表面の実測（実戦 1,094 手）**: 露出スロット数 **平均 5.5**、露出している型の種類数 **平均 4.97**（＝**既に 90% が異なる型**）、next が露出している手 40%。ジャンク（T1–T2）が占めるスロットは 12%（0.7 スロット/手）だけ。型別の露出頻度は T11 59% / T8 48% / T10 47% / T6–T7 43% / T9・T12 40% / T1–T5 30–36% / T13 8%。
- **v757 の伸びしろの再評価**: 理論上限は「全スロットが異なる型」＝ 5.5/11 = 50%。現状 4.97/11 ≈ 45%。**上限まででも +5pt（→ +0.038 併合/手）**で、当初試算の +0.05 より小さい。それでも v748 の実績（+0.038）と同等なので実戦 A/B に出す価値はある。
- **「スロット数を増やす」方向は観測では支持されず**: 露出スロット数と 併合/手 の相関は **−0.30**（多いほど悪い）、露出駒の平均サイズとは **+0.41**（大きいほど良い）。ただし n=14 かつ内生（併合が進んだ試合ほど大きい駒が表面に出る）で、因果とは読めない。**観測データでの機構探索はここで限界**。以後は A/B でしか判別できない。
- 方針: 余計な候補を増やさず、**v757 の実戦 A/B に集中**する。結果が出てから次の設計を決める。

## 2026-08-30 01:1x-01:4x JST — analyzer_modes 記録を確認 / 露出の寿命は 3〜4 手（v757 の期待効果を試算）

- **解析器 A/B（01:30、k=35、n=71/71）**: score −119（UCB90 +48）、併合/手 −0.0224（CI90 [−0.042, −0.002]）、**T15 4 vs 5（B が逆転）** → 継続。k=50 は ~06:00。
- **`analyzer_modes` の記録を実測確認**: A/B 記録 11 件で `('A',0): 6, ('B',3): 5` と腕別に正しく分離。今後の解析器 A/B は履歴だけで事後検証できる。
- **露出の寿命（実戦 1,499 エピソード、`expo_life.py`）**: 露出が続くのは**中央値 3〜4 手**（平均 3.7〜4.6、p90 8〜10）、露出中に併合できた割合は 32〜37%。x 位置別の差は小さい（壁際がわずかに長い: 中央値 4 手・37% vs 中央 3 手・32%）→ **「壁際に置けば露出が長持ちする」類の精緻化は期待薄**。
- **v757 の期待効果の試算**: 露出寿命 ~4 手 → ある型が露出中に出番を迎える確率は 1−(10/11)^4 ≈ 31%。露出型数が 3.8 → 4.5 に増えれば「next が露出している手」は 38% → ~45%、併合率 76% を掛けて **+0.05 併合/手** 相当（v748 の +0.038 を上回る）。容量モデルでは 併合/手 0.56 → 0.61 ＝ 手数 95 → 108。ただし定常状態での露出型数がどれだけ増えるかは実戦でしか測れない。
- 次 tick 以降: status のみ。k=50（~06:00）で判定 → `finish` → **v757 vs v752 の長期 A/B**（`tmp/manual_challenge/strategy_351b06dae2bd.py`）を開始し、音声進捗で報告。
...
## 2026-08-29 23:4x-23:5x JST — AIラジオの配信実装説明を「必要時の限定参照」へ変更

- **ユーザー指摘**: 特定製品名を断定禁止にするだけでなく、配信実装へ触れるならhandoffやVM上のコードを確認して判断すべき。一方、運用情報の本文を常時プロンプトへ注入すると、無関係な話題を汚しコンテキストウィンドウを消費する。
- **修正**: AI知識コーナーでコーディングエージェント情報を常時取得する処理を廃止し、`AIエージェント`・`コード生成AI`・`AIワークフロー自動化`の関連テーマだけに限定。そのテーマ内でも配信実装へ触れる必要がある場合だけ、生成エージェント自身がRead/Grepで `prompts/ops_brief.md`（docich `handoff.md`由来）、`core/config.sh`、`broadcast/radio_engine.sh`、`workers/radio_worker.sh`、`strategy/ai.sh` の該当箇所を読む。VM `.env` は全文を読まずラジオ経路4キーだけをGrepし、既定値と突合する。ファイル本文のプロンプト注入はしない。
- **検証と反映**: `tests/test_ai_knowledge_runtime_neutral.sh` に関連/無関係テーマの境界、本文注入ヘルパー不在、安全な`.env`限定確認を追加。ローカル・VMで対象テストとbash構文成功、VMコードのRead権限を確認し、3ファイルのSHA256一致。soviet_now `856e83284` をpush・VM反映、radio worker PID `528038` が23:49:25にUSR1 reload complete。次回の関連AI知識コーナーで実際に必要時Read/Grepが行われ、正確な説明が再生されることは未確認。

## 2026-08-29 23:2x-23:3x JST — AIラジオが配信全体をClaude Code製と断定する説明を停止

- **実害と根因**: AI知識コーナーの生成指示が「このゲーム配信自体がClaude Codeで動いていることにも触れてよい」「この配信自体がClaude Codeで動いている」と明記していた。修正直前の実原稿も「この配信そのものはAnthropicのClaude Codeの上で動いています」と生成し、23:23:26〜23:29:30に実再生済みだった。実際のラジオ生成・運用は複数エージェントのフォールバック経路を持つため、この固定説明は不正確。
- **修正**: `broadcast/radio_corners.sh` では各製品を業界のツール例としてのみ扱い、Claude Code・Codex・OpenCode等を現在の配信全体の実行環境として推測・断定しないよう変更。現在の担当製品は生成プロンプトから特定できないことも明記。回帰テスト `tests/test_ai_knowledge_runtime_neutral.sh` を追加。soviet_now branch `codex/fix-news-date-freshness` commit `2f8ac2ef9`、push・VM反映済み。
- **検証**: ローカルとVMで対象テスト、bash構文、diff check成功。VMの `broadcast/radio_corners.sh` はローカルとSHA256一致。radio worker PID `528038` はUSR1後も同一PIDで稼働し、23:36:40に `reload complete` を確認。既に再生済みの当該原稿は履歴として残し、待機キューの削除や過去原稿の改変はしていない。次回以降の実AI知識コーナーで新しい中立表現が実際に生成・再生されることは未確認。

## 2026-08-29 23:1x-23:5x JST — 直落とし経路は取り切り済みと確定（残りは正しくフィルタ）/ 解析器 A/B は害停止の直前

- **解析器 A/B（23:29、k=29、n=58/58）**: score −162（SE 154、**UCB90 +34**）＝害停止（UCB90<0）まであと 1〜2 ブロック。併合/手 −0.0284（CI90 [−0.051, −0.006] で再び全域マイナス）、T15 4 vs 3。
- **取りこぼし 7% の内訳（実戦、露出同型があるのに落とさず併合もしなかった 69 手）**:
  - **B: relief>1.0 が 52%（36 手）** — 隣が高く成功率 20% 域なので**落とさないのが正しい**
  - A: 解析器が別に DIRECT/NEAR を出していた 17%（12 手） — 別経路を選んで失敗しただけで直落としの問題ではない
  - E: 候補が交差/壁回転リスク 12%（8 手）、C: 締切余裕不足 4%（3 手） — いずれも安全フィルタが正しく働いている
  - **F: 条件を満たすのに他軸が競り勝った 14%（10 手）= 全手の 0.8%** — 唯一の回収余地だが 88% 併合としても **+0.007 併合/手** で無視できる大きさ
- **結論**: **「露出同型への直落とし」経路は既にほぼ取り切っている**（v742/v744 の OPEN_TWIN が機能している）。「もっと積極的に取る」方向の候補は作らない。**残る唯一のレバーは、露出同型がある手の割合（現在 38%）を上げること＝v757 SURFACE_DIVERSITY の方向**。
- 次 tick（00:13 以降）: 害停止が成立していれば `finish A`（`.env` 不変）→ **v757 vs v752 の長期 A/B を開始**し、音声進捗で報告する。成立していなければ k=50（~06:00）まで継続。

## 2026-08-29 22:4x-23:1x JST — ラジオ本文内の二重時刻を除去し、実再生で単一時刻を確認
...
- **解析器 A/B（22:29、k=25、n=51/52）**: score −117（UCB90 +107）、併合/手 −0.0214（CI90 [−0.044, +0.001]）、T15 4 vs 2 → 継続（k=50 は ~06:00）。
- **併合経路の定量化（実戦 14 試合、露出同型がある 484 手）**: 全体の併合率 76%。**その同型の真上（±0.4）に落とした 206 手（43%）は併合 88%**、落とさなかった 278 手は 67%（ただしその多くは別の駒との DIRECT 併合を選んでおり「取りこぼし」ではない）。純粋な取りこぼしは 278×33% ≈ 92 手 ≈ 全手の 7%。見送りの理由上位は `AVOID_BLOCK_REACTIVE_PAIR_T12_CHAIN_LANE_GUIDANCE`（53）。**この 7% を取り切れば +0.03 併合/手 相当**だが、選択バイアス（良さそうな時だけ落としている）があるので上限値。
- **干渉チェック（重要）**: v757 は「露出を覆わない」方向に加点するので、**同型の真上への直落とし（＝相手を覆う手）を抑制する恐れ**があった。実戦 480 手で v752 と v757 の決定を比較した結果、**同型の真上に落とす率は 50% で同一、`OPEN_TWIN_MERGE` タグも 14% で同一** → 干渉なし。v757 の加点は `merge_grade == NO` かつ安全条件下でのみ効き、直落とし経路は別ルート（ラッパー）なので競合していない。
- 次: 解析器 A/B の k=50 判定（~06:00）→ **v757 vs v752 の長期 A/B**（`tmp/manual_challenge/strategy_351b06dae2bd.py`、validator OK）。その後の候補として「露出同型がある手で AVOID_BLOCK 系より直落としを優先する」変種（上記 7% の回収）を検討。

## 2026-08-29 21:1x-21:4x JST — **v757 SURFACE_DIVERSITY を実装、機構をオフラインで確認**（初めて狙いどおり動いた候補）

- **解析器 A/B（21:29、k=22、n=44/44）**: score −79（UCB90 +173）、併合/手 −0.0112（CI90 [−0.037, +0.014]）、T15 4 vs 2 → **中立に回帰**。k=50（~06:00）まで継続。
- **v757（hash `351b06dae2bd`、`selfplay_root/strategy_v757.py`、v752 ベース）**: 事前計算で「露出している駒」を型ごとに集め、各候補について**落下後に露出したままの型の種類数**を計算。`score += 200 × Δ(露出型数)`（増える候補に加点、tag `SURFACE_DIVERSITY`）。落とす駒自身が新しい型を表面に出す場合も +1 と数える。安全条件は他軸と同じ。
- **設計根拠（本日の実測）**: (1) 寄せは不可能（近接ペア 5573 件の 3 手以内併合 3%、ドロップ位置無関係）(2) 埋没は 27% が埋没のまま併合＝「覆わない」系は弱い (3) **next_type の同型が露出している手は 38% でその併合率 76%**。表面に並ぶ型の種類数がそのまま併合機会になる。
- **オフライン検証**: 対 v736 コーパスで changed 5089/11155、**DIRECT 損失 0、newcross 116（v752 と同数＝終盤併合の意図的超過のみ）**、risk_top −0.178。**機構検証（実戦 700 手で v752 と決定を比較）: 1 手あたりの露出型数の変化が v752 −0.561 → v757 −0.366（損失 35% 減）、タグ発火 20.1%**。v754/v755 が機構を逆に動かしたのと違い、**狙いどおり動いた初めての候補**。fixture テスト 6 件合格。VM 配置・validator OK。
- **音声進捗を投入**（節目報告）: 寄せが不可能なこと、終了時の駒の 96% が回収不能なこと、併合の唯一の経路（38% / 76%）、v757 の機構改善（0.56 → 0.37）を視聴者向けに説明。
- 次: 解析器 A/B を k=50 で判定（~06:00）→ `bash tools/ab_ctl.sh finish A|B` → **v757 vs v752 の長期 A/B を開始**（`tmp/manual_challenge/strategy_351b06dae2bd.py`、事前登録は従来どおり）。

## 2026-08-29 20:1x-20:5x JST — **「寄せ」は物理的に不可能と確定 / 終了時の駒の 96% は相方が盤上にいる** — 併合の唯一の実効経路を特定

- **解析器 A/B（20:29、k=18、n=37/38）**: score −219（SE 220、UCB90 +63）→ 害停止せず。併合/手 −0.0266（CI90 [−0.055, +0.002]）。k=12 −249 → k=15 −283 → k=18 −219 と 0 に回帰中で、当初の負は一部ノイズだった。規則どおり k=50（~06:00）まで継続。
- **否定結果 1: ナッジ（寄せ）は効かない**（`nudge_probe.py`、実戦 14 試合の近接同型ペア 5573 件）。gap 0.05–1.2 のペアが 3 手以内に併合したのは **3%** で、ドロップ位置（間の真上 / どちらかの真上 / 左右外側 / 遠い）で **2〜4% とほぼ無差**（「遠い」が 4% で最高）。→ **settled した駒は落下の衝撃では動かず、離れた同型ペアは実質的に永久に併合しない**。以前に検討した REACTIVE_NUDGE 系は破棄。
- **否定結果 2: 埋没は主因ではない**（`burial_life.py`、902 エピソード）。埋没は 1 試合 64 回起きるが、**26% は解放（中央値 2 手）、27% は埋没したまま併合**（側面接触で併合できる）。→ v743 LAST_EXPOSED / v750 EXPOSURE_KEEP が中立だった理由が説明できる。「覆わない」系の軸は原理的に弱い。
- **損失の実像**: 終了時の駒 42.6/試合のうち **96%（41.0）が「同型の相方が盤上にいるのに併合していない」**駒。真の単独は 1.6 のみ。未併合ペアの最小間隔は 接触 3.8 組 / 近い(≤1.5) 4.1 組 / 遠い 3.3 組（中央値 0.73）。だがナッジが効かない以上、**これらは回収不能**。
- **併合の唯一の実効経路**: 新しく落とすピースが**露出した同型**に届くこと。実測: next_type の同型が露出している手は **38%**、**その手の併合率は 76%**。露出していない手ではほぼ併合しない。→ 改善は「露出した同型に届く手の割合を 38% から上げる」に尽きる。ただし試合単位の「露出型数」と 併合/手 の相関は −0.04（内生的に交絡: 併合が多い試合は駒が減り露出型数も減る）ので、**試合単位の観測では検証できず A/B が必要**。
- **次候補の再検討**: v756（v752＋EXPOSURE_KEEP）は「覆わない」系なので、否定結果 2 より期待値は低い。代わりに **「落とす駒自身が露出したまま残る位置を選ぶ」**（自分の露出を最大化＝将来 next で届く相手を増やす）方向の候補を設計する価値がある。解析器 A/B 終了（~06:00）までに設計・オフライン検証する。

## 2026-08-29 20:2x-20:3x JST — ポッドキャストにもニュース分野フィルターを適用、既存11件を追加隔離

- **既存素材**: 8月29日の通常配置news 29件を再確認し、スポーツ3件、映画・テレビ・芸術3件、Apple製品発表1件、SpaceX飛行試験2件、ゲーム記事1件、RRR舞台化1件の計11件をVM `.podcast_excluded/` へ追加移動。以前の7件と合わせて隔離18件、通常配置newsは18件。削除はしていない。
- **恒久対策**: `podcast_build.py` がnews原稿の冒頭700字を再検査し、sports、芸能・映画・音楽、製品/レビュー/セール、SpaceX/Starship等の対象外科学記事を収集時にも除外する。原稿後半の比喩に出る「芸人」等で公共性記事を誤除外しないよう冒頭だけに限定。soviet_now `fba9347df` / `af937355b` / `ce75d9558`、push/VM反映済み。
- **検証**: テスト24件、py_compile、zsh構文、diff check成功。VM実収集は news 18 + jiji 32 = 50件、隔離先の混入0。Macの原稿ミラーはまだ当日早朝分のみだが、同期済みApple/Starshipの2件を収集フィルターが実際に除外し、news 4件だけを残すことを確認。外部原稿ミラーへの手動pushは安全確認で行わず、Mac checkoutはcleanに戻した。次回VM定時アーカイブ同期で隔離配置がミラーされる。

## 2026-08-29 20:0x-20:2x JST — ニュース選定を社会・政治・国際情勢・経済に限定

- **原因**: 前回追加したGoogle News総合・科学カテゴリから、4Gamer系の製品記事、芸能、商品紹介などが候補へ混入。変更直前には「宝塚歌劇団星組公演」が実際に選定・生成された。
- **変更**: Google Newsは日本社会・日本政治・日本経済・国際の4専用フィードだけに限定し、総合・科学を削除。一般速報の朝日/毎日RSSも外した。Google候補には公共性語（政策、事件、災害、外交、労働、医療、市場、投資等）を必須化し、芸能・映画・音楽・著名人私生活・生活情報・新製品/レビュー/セールと娯楽/販促媒体を除外。多言語World Cupもsports扱いにした。取得時、キャッシュ復元時、放送直前の3段で防御し、生成プロンプトにも対象分野を明記。soviet_now `0987fc650`、branch `codex/fix-news-date-freshness`、push/VM反映済み。
- **実測**: ローカル実RSS 14/14ソース、536件取得。古い176、sports 8、低価値18、公共性不足50等を落として281候補。VMでは既読込みで269候補を確保し、先頭群は災害警報、欧州強硬右派、ウクライナ国防、金融、外交等。放送直前関数の副作用なし選定は「勝山市のレベル4土砂災害危険警報」。テスト21件、py_compile、bash構文、diff check成功。workerは各iterationで`eloop_lib.sh`を再読込するため、進行中の時事生成を中断する再起動は行わず、次iterationから新定義が有効。
- **既存素材**: 変更直前の宝塚原稿 `radio_1787999388_46808_news_24075.txt` は削除せず `.podcast_excluded/` へ移動。20260829の隔離は計7件。

## 2026-08-29 19:1x-19:4x JST — 解析器 A/B の劣化原因を特定（安全ゲートが開いて自作評価軸の発火が増える）/ 害停止まであと僅か

- **成績（19:29、k=15、n=31/30）**: ブロック score **−283（SE 246）→ UCB90 +32**（害停止 UCB90<0 まであと 1〜2 ブロック）。併合/手 −0.0363（CI90 [−0.069, −0.004]、全域マイナス）、T15 4 vs 1。
- **劣化の機構（game_history、mode 別 1429 手 vs 1236 手）**: mode 3 は狙いどおり **締切余裕 2.76 → 2.90**（負の割合 4.9%→4.5%）、**超過ドロップ 8.9%→7.0%**、落下後上端 0.87→0.57、risk 0.92→0.62。しかし副作用として、**私が追加した評価軸の安全ゲート（`reactor_margin >= 1.0` / `not deadline_crossed`）が開きやすくなり発火率が上がった**: SEAT_LANE 29.1%→**33.9%**、LAST_EXPOSED_COVER_AVOID 14.4%→**18.2%**、OPEN_TWIN_MERGE 4.2%→5.7%、逆に DEADLINE_GUARD 4.6%→3.7%。
- **含意（重要）**: 解析器の締切モデル自体が悪いのではなく、**現在の評価軸は今の発火率に合わせて調整されており、発火を 4pt 増やすと悪化する**＝軸は最適点付近かやや過剰。今後は「置き方ボーナスをもっと強く／もっと広く」方向では伸びない。次の伸びしろは**新しい機構**（まだ触れていない損失源）に求めるべき。
- **今後の A/B 設計への反映**: 解析器を変える実験は、戦略側の安全ゲートを通じて自作軸の発火率を動かすため、**単独の変更として解釈できない**。解析器変更を評価するなら、発火率を固定する（ゲートの閾値を margin ではなく生の高さで持つ等）か、素の v736 系で行う必要がある。
- 次 tick（20:13）: 害停止判定（UCB90<0 なら `finish A`、`.env` は不変のまま）→ 成立すれば v756 の長期 A/B を開始し、両方を音声進捗で報告する。

## 2026-08-29 19:0x JST — ポッドキャスト素材から非ニュース・未確認・欠落原稿6件を隔離

- **対象**: 2026-08-29分の保存済み news 原稿33件を本文と生成履歴で確認。冒頭欠落1件、生成中に別話題へ言い直した原稿1件、2024年の太陽探査回顧1件、「最近のニュースでもなんでもない」と明記した1963年の回顧1件、1949年RDS-1回顧1件、当夏の具体的成果を「確かめ切れていない」と明記したJWST原稿1件の計6件を除外した。
- **処置**: 削除せず、VM `/home/ubuntu/soren/backups/radio_scripts/20260829/.podcast_excluded/` へ移動。Macの非公開原稿ミラーに既に同期済みだった冒頭欠落原稿1件も同名隔離先へ移し、`azumag/soren-radio-archive` main `e42365c` へpush済み。残る5件はVM側で隔離済みのため、次回アーカイブ同期時も通常の日付直下へは入らない。
- **検証**: VM日付直下は news **27件**、隔離先は6件。`podcast_build.collect_files(2026-08-29)` の実行結果は news 27 + jiji 29 = **56件**、隔離先を拾った件数は **0**。通常ニュースは大半を残しており、候補切れにはしていない。生成済みポッドキャストはまだ存在しないため再生成は不要。

## 2026-08-29 18:5x JST — 音声進捗報告の欠落を指摘され再開（運用ルールをメモリ化）

- **指摘**: 改善・テストの節目で `./system_progress_report.sh` による音声進捗を出していなかった（バナー `codex_work_indicator.sh` は使っていた）。CLAUDE.md / AGENTS.md の明記事項で、**私の運用漏れ**。
- **対応**: v752 採用と解析器 A/B の内容を音声進捗として投入（`tmp/.comment_queue/comment_announce_*_system_progress.txt` を確認）。今後は**節目ごとに 1 回**（候補実装完了 / A/B 開始 / A/B 判定 / 根本原因の発見 / 方針転換）出す。tick ごとの status 確認では出さない（キューを埋めないため）。メモリ `soren-progress-report-duty` に運用を記録。
- 文面ルール: 視聴者向けの日本語、数字（試合数・差・信頼区間）を入れる、VOICEVOX が読むので記号・英略語を避ける。完了報告 → `codex_work_indicator.sh stop` の順。

## 2026-08-29 18:1x-18:4x JST — 去年/未確認ニュースの「本日」扱いを停止、公開日時付き候補を復旧

- **実害確認**: 16:55生成のニュースで、RSS未読候補が0件になった後のAI自主探索が「1949年8月29日のRDS-1実験」を本日のニュースとして生成した。該当入力はRSS記事ではなく、モデルが日付一致の歴史題材を選んだもの。修正前ログは `sources=9/9, fetched=100, candidates=0` で全100件が既読扱いだった。
- **根因**: (1) 候補切れ時にモデルの記憶でニュースを作る `_news_self_search_fallback` が既定有効、(2) `fetch_news.py` が実際に読んだ1件ではなく取得候補全体を既読へ追記して候補を枯らす、(3) 公開日時の最大経過時間フィルタが無かった。
- **修正**: `NEWS_SELF_SEARCH_FALLBACK=0`（既定）、`NEWS_MAX_AGE_HOURS=48`。Google Newsの日本・政治・経済・国際・科学RSSを追加し、公開日時なし/48時間超を除外。既読化は `radio_news.sh` が実際に選んだ1件だけに統一。プロンプトへ出典と公開日時も明記。soviet_now branch `codex/fix-news-date-freshness`、commits `5ddfdd22c` / `35040be04` / `edd0bff74`、push済み。
- **検証**: ローカル/VMで鮮度テスト3件、旧自主探索互換テスト全件、py_compile/bash構文/diff check成功。VM実RSSは **17/17ソース、578件取得、48時間内未読候補375件、選定可能228件**。同じ本番選定関数で `首相、物価高対策前のめり... - 日本経済新聞`、出典 Google News 日本政治、公開日時 `2026-08-29T07:58:58Z` を選ぶことを確認。
- **worker復旧**: 古いradio生成子シェル43件をユーザー承認後に終了。pause marker解除後、radio worker PID `528038` が supervisor PID `856212` 直下で18:42起動。`soren-runtime.service=active`、stream 1系統、ゲームrunner継続。18:50に実ニューストリガーを1回投入し、Google News科学の4Gamer記事（公開 `2026-08-29T04:21:52Z`、選定時約5.5時間）を実経路で選定。**原稿生成・実読み上げは未確認**（時事ニュース調査がAIキューを占有し、ニュースはスパム判定待ち）。選定経路・候補鮮度は確認済みだが、実際の音声で新記事が流れるまでは視聴者可視の完了とはしない。
...
- **成績（17:29、k=8、n=17/17）**: score A 2066（med 1761、101.7 手）/ B 1822（med 1372、98.2 手）、ブロック mean(B−A)=−208（SE 440）→ **UCB90 +355 > 0 なので事前登録の害停止に該当せず継続**。併合/手 −0.0577（z −1.57）、T15 2 vs 1。
- **機構検証（game_history に残る 27 試合、landing_arc 別）**: mode 3 は **超過フラグ付きドロップ 10.5% → 7.8%**、終盤 5 手の超過 77% → 64% と**設計どおり誤警報が減っている**。同じサンプルでは手数 96.6 vs 90.0（mode 3 が長い）、終了時駒数 40.3 vs 38.1（容量 C が大きい）だが 併合/手 は 0.531 vs 0.562（低い）。容量モデル `手数 = C/(1−m)` で相殺され、手数の差は誤差範囲。**A/B 記録（17/17）では B のほうが短い**ので、game_history 側の 13/14 試合サンプルとは符号が逆＝どちらも n 不足。
- **判定規則の扱い**: この実験の事前登録は **score ベース**（害停止 UCB90<0、k=50 で CI90 下限 >0）。主要指標を 併合/手 に変えたのは「次の A/B から」と宣言したので、**途中で規則は変えない**。ただし 併合/手 が z −1.6 前後で推移し続けるなら score 側も k=12 前後で無益/害の域に入るはず。**`.env` は未変更（本番＝mode 0）**なので、棄却されても後処理は不要。
- 次 tick（18:13）: status のみ。k=50 は明日 05:00 頃。

## 2026-08-29 16:1x-16:4x JST — 解析器 A/B: 計測は正しいが B（mode 3）が負に振れている（k=5、害停止判定は k≥6）

- **計測の妥当性を実測確認**: 直近 10 試合の `analyzer_modes.landing_arc` と `[AB] idx=... arm=...` を突合し、**B 腕の試合はすべて 3、A 腕はすべて 0** で完全一致。腕別 env は runner → analyze_board まで届いている。（`tmp/state/ab_games.jsonl` の `analyzer_modes` が None なのは `_ab_record_game` がこの項目を拾っていないため。検証は game_history 側で行う。）
- **成績（16:29、k=5、n=11/10）**: score A 2289（med 1845、107.8 手）/ B 1746（med 1310、99.2 手）、ブロック **mean(B−A)=−597（SE 584）→ UCB90 +150 > 0 なので害停止には該当せず**。併合/手 −0.068（z −1.41）。
- **解釈と注意**: オフライン検証（締切の誤警報 25→20、見落とし 0）は「落下ピースの上端が締切線を超えたか」で測ったが、**実際のゲームオーバー条件はそれと同一とは限らない**（併合結果の押し上げ、静止判定など）。局所自己対戦と同じ轍で「オフラインで良い＝実戦で良い」ではない可能性がある。事前登録どおり k≥6 で UCB90<0 なら即 `finish A`（mode 3 不採用、`.env` 不変）、そうでなければ継続して k=50 判定。**現時点で `.env` は変更していない**（既定 0 のまま、A 腕＝本番挙動）。
- 次 tick（17:13）: k≥6 の害停止判定を実施。
...
- **締切予測の精度（実測、選択された落下 850 手）**: 従来 = 予測 crossing 52 件・実際 27 件 → **誤検出 25、見落とし 0（適合率 52%）**。→ 締切判定は「安全側に倒す」設計だが、過剰。
- **修正と検証（3 モード）**:
  - mode 1（着地も円弧）: 着地誤差 +0.689 → +0.493、締切 誤検出 25 → 20。**ただし併合予測が 88.8% → 81.4%（見落とし 133 → 226）に悪化**（`hit_id` が変わり DIRECT が落ちる）→ 不採用。
  - mode 2（着地は従来、接触判定だけ `LANDING_SINK_CALIB=0.60` 沈める）: 併合 88.8% → 89.2%（見落とし 133 → 127）。沈み量を 0.35〜1.10 で振っても 89.3% で頭打ち → 効果は小さい（+0.005 併合/手 相当）。
  - **mode 3（締切の着地だけ円弧）: 併合判定は完全に不変（88.8%、見落とし 133、誤検出 6）、締切の誤検出 25 → 20、見落とし 0 維持（適合率 52% → 57%）。全候補が超過になる手 6.8% → 4.8%、DIRECT があるのに全 DIRECT が超過扱いの手 13.3% → 8.5%。** → 採用候補。
- **commit**: sn-mine `281ff93d9`（`analyze_board.py` に `landing_arc_mode()`＋円弧式、既定 0 で従来と完全同一。`tests/test_landing_arc_mode.py` 5 本）。関連テストは変更前 29 failed → 変更後 25 failed（既存の失敗のみ、新規破壊なし）。**VM・本番は未変更**。
- **次の課題（実戦検証の方法）**: 解析器モードは env なので現行 A/B（戦略ハッシュ単位）では腕ごとに変えられない。`strategy/ab_interleave.sh` の `_ab_select_arm` で `AB_EXTRA_ENV`（例 `ANALYZE_BOARD_LANDING_ARC=3`）を export し、`eloop.sh` の `play_one_game` が runner 起動時にそれを適用する小改修が要る。v752 の A/B 終了後（15:00 以降）に実装 → 解析器 A/B を開始する。

## 2026-08-29 10:1x-10:5x JST — **解析器の着地モデルに系統誤差 +0.69 を発見**（終盤の併合拒否・併合見落としの共通根因の候補）

- **v755 FLAT_PROFILE も機構が逆**（局所 18+18）: `FLAT_PROFILE` 11.9%/手 発火、しかし 25 手目の roughness 0.160（v748 0.136）・最高 y −1.88（−2.26）・露出率 0.405（0.475）で**すべて悪化**。v754 と同じパターン。原因を調べたところ、私の roughness モデルが `top_y_after_drop`（実測より +0.73 高い）を使っていたため、全候補の列天端が同じ高さに張り付いて事実上ノイズになっていた。
- **系統誤差の実測（実戦 14 試合 852 手、解析器で再計算 vs 次スナップショットの実着地）**: `landing_y − 実着地 y` = **平均 +0.69、中央値 +0.60、sd 0.61、95% が過大**。内訳: 駒の上に着地 n=834 で **+0.72**、床着地 n=18 で −0.59。壁際 +0.40 / 非壁際 +0.74、横ずれ ≤0.3 で +0.61 / >0.3 で +0.76、駒数 10 台 +0.81 → 40 台 +0.43。型別は 0.56〜0.82 でほぼ一様。
- **原因（コード）**: `analyze_board.get_landing_info`（622–644 行）は、横方向の外接が少しでも重なる相手すべてについて `collision_y = p.y + p_top + drop_bottom` とし、その最大を着地とする＝**箱積みモデル**。実際の円/多角形は接触円弧に沿って谷へ滑り込むため、横ずれが大きいほど実際は低く止まる。
- **影響（仮説→一部実証）**: (1) 締切超過の予測が過剰に保守的（`margin` 0.5–1.0 の「越える」落下で 2 手以内ゲームオーバーは 0.1%）→ ユーザー指摘「終盤に併合できるのに安全着地して死ぬ」。(2) 併合の接触予測がずれる → 併合予測の見落とし **133/1242 手（10.7%）**（露出双子への直落としが実際は 60–80% 併合する、という 08-27 の測定と整合）。
- **第一次修正と結果**: `ANALYZE_BOARD_LANDING_ARC`（既定 0＝従来、本番・リポジトリ未変更、局所 `selfplay_root/analyze_board.py` のみ。原本 `analyze_board.py.pre_arc`）。円弧接触: `dy = r_v × sqrt(1 − dx²/r_h²)`（r_h = 横方向接触距離、r_v = 縦方向接触距離）。**結果: 着地誤差 +0.689 → +0.493（改善）だが併合予測の正答率 88.8% → 81.4%、見落とし 133 → 226（悪化）**。→ 着地高さだけ下げると、併合判定（接触テストが箱積み前提）が相手を取り違える。**着地と接触判定を対で直す必要がある**（次 tick）。
- 実戦 A/B（v752 vs v748）10:30: 各 72、併合/手 +0.0089（z 0.61）、score +104、T15 2 vs 3。

## 2026-08-29 09:1x-09:4x JST — v755 FLAT_PROFILE（盤面凹凸を直接評価）を実装・機構検証中

- **実戦 A/B（A=v748 / B=v752）09:29**: 各 65/66、score 1770 / 1910、**併合/手 0.552 / 0.565（差 +0.0127、SE 0.015、z 0.84）**、手数 96.8 / 101.4、T15 1 vs 3 → 継続（k=50 は 15:00 頃）。
- **v755（hash `e1060960b976`、`selfplay_root/strategy_v755.py`、v748 ベース）**: 新ヘルパ `strategy_helpers/profile.py`（additive）で x∈[−3,3] を 12 列に分け各列の天端 y を取り、**roughness = 隣接列の段差の平均**を定義。非併合候補について「落下後の roughness − 現在の roughness」を計算し、`score -= 300 × Δroughness`（平らになる候補に加点、tag `FLAT_PROFILE`）。安全条件は他軸と同じ。v754（着地 y のペナルティ増幅＝逆効果）とは違い、**盤面の凹凸そのものを評価**する。オフライン（対 v736）: changed 3955/11155、DIRECT 損失 0、newcross 0、risk_top −0.225。
- **機構検証 run**: `tmp/selfplay/v755_m`（A=v748 / B=v755、3 slots × 12、FPS=30、ports 20380/19140）を 09:35 起動（~11:00）。**判定は機構のみ**（25 手目の最高 y と高低差が下がるか、露出率が上がるか、DIRECT 損失 0）。スコア差は局所では意味を持たないので採否には使わない。
- 終了 run の `slot-*` を削除（自己対戦データ 1.0 GB → 走行中 run 含む）。

## 2026-08-29 08:1x-08:4x JST — 分散削減は不可（ピース列の運は分散の 3%）/ 高さペナルティ増幅は平坦化に効かない（機構レベルで確認）

- **実戦 A/B（A=v748 / B=v752）08:29**: 各 58/59（idx=0 除外）、score A 1785 / B 1931（差 +146、SE 124、z 1.18）、**併合/手 0.554 / 0.564（差 +0.0092、SE 0.016、z 0.56）**、手数 97.4 / 102.0、**T15 1 vs 3** → 継続（k=50 判定は 15:00 頃）。
- **分散削減の検証（420 試合）**: ピース列の運は戦略と独立な外生変数なので共変量に使えるはずだったが、**先頭 40 手だけで作った厳密に外生な特徴（連続同型・1/2 つ飛ばし同型・重複率・最頻型比・エントロピー・低型/高型比）の重回帰 R² = 0.032** → 必要 n は 97% にしか減らない。（全期間のエントロピーで見た +0.33 の相関は生存長に依存する内生量だった。）→ **併合/手 の分散 (sd 0.107) は物理・打ち手側の内在ノイズ**で、共変量調整では減らせない。必要 n は据え置き（+0.05 で 66/腕、+0.03 で 183/腕）。
- **v754 の機構解析（局所 3000 手）**: `EARLY_FLATNESS` は 44%/手 で発火しているのに、**落下後の上端は −0.36（v748 −0.41）と高くなり、25 手目の最高 y も −1.83（−1.97）、高低差も 2.25（2.08）と悪化**。→ 「高さペナルティを増幅すれば盤面が平らになる」という仮定は誤り（最低点ばかり狙うと x 方向の質＝座席・併合準備を無視し、結果的に山ができると推測、機序未確定）。**この方向のレバー（height_penalty の係数）は捨てる**。平坦さ自体は高併合試合の特徴（25 手目 −1.90 vs −1.51）なので、別の作り方（例: 落下後の「盤面の高低差」を直接評価軸に入れる）で再挑戦する余地はある。
- 次: v752 の k=50 判定（15:00 頃）。それまでは実戦枠が空かないので、(a) 高低差を直接評価する v755 の設計と機構検証、(b) A/B 多腕化（ABCD、スループット 1.5 倍）の実装、のどちらかに着手する。

## 2026-08-29 07:1x-07:4x JST — **局所自己対戦は採否に使えないと確定**（併合/手でも同じ）/ v754 は機構が逆方向で棚上げ

- **較正（既知の実戦結果と局所の併合/手を突合）**: v741 局所 +0.0425（SE 0.018）→ 実戦 score −528（害停止）／v744 局所 +0.0422（SE 0.020）→ 実戦 −39（無益停止）／**v748 局所 +0.0552（SE 0.013）→ 実戦 +0.038（採用）**。3 候補すべて局所は正で、実戦の符号を当てたのは 1 つだけ。**局所は指標を替えても順位付けできない**（score でも 併合/手 でも同じ）。→ 運用: **局所 = 機構検証専用**（タグ発火率・意図した盤面指標が動くか・DIRECT 損失ゼロの確認）、**採否 = 実戦 A/B のみ**（併合/手 主要、各腕 66–100 試合、10–15 h/候補）。
- **v754 EARLY_FLATNESS（9a1013eedf1e）の機構検証結果**: 局所 27+28 試合で 併合/手 0.561 vs 0.545（+0.016）、手数 100.8 vs 99.3、score 1881 vs 1797 だが、**狙った指標が逆**: 25 手目の最高点 −1.83（v748 −1.99）＝**平らにならず高くなった**、露出率 0.400（v748 0.417）＝**下がった**。ブロック差も P −375 / Q −349 で効果 ≈ −13。→ **棚上げ**（高さペナルティ ×2.5 は landing_y を下げる方向に働くはずだが、谷埋めの結果として端が高くなる／候補集合が変わるなどで意図と逆に出た。原因未特定）。
- **実戦 A/B（A=v748 / B=v752）07:29**: games=105（各 52、idx=0 除外）、score A 1732 / B 1896（k=25 +192、SE 150）、併合/手 A 0.548 / B 0.559（+0.0115、z 0.67）、手数 95.9 / 101.4、T15 1 vs 2 → 継続。
- **含意（スループット）**: 採否が実戦のみ・1 候補 10–15 h なので、候補の質を上げるか、A/B を多腕化（ABCD で 3 候補同時、各腕 n は 1/2 になるが 3 倍の候補を評価）するかが次の infra 課題。多腕化は `strategy/ab_interleave.sh` の `_ab_select_arm` とレポート/判定の拡張が必要（進行中の A/B 終了後に着手可）。

## 2026-08-29 06:2x-06:4x JST — 連鎖選択は最適済み / 高併合試合は「平ら・露出高」→ v754 EARLY_FLATNESS を作成

- **実戦 A/B（A=v748 / B=v752）06:29**: games=93、score A 1748 / B 1868、k=22 +122（SE 151）。**併合/手 A 0.548 / B 0.556、差 +0.0075（SE 0.018、z 0.42）＝中立**。T15 各 1。idx=0（A/B 開始前に始まった v736 の試合、hash 253cc67e0c1b）は判定時に除外する。
- **連鎖併合の診断（実戦 14 試合 320 手、`chain_diag.py`）**: DIRECT がある手の 32% で「生成物が既存 N+1 に接する」連鎖選択肢が存在し、**戦略はその 98%（95/97）を既に選んでいる**（逃したのは 2 件）。連鎖を選んだ手の複数併合率 55% vs 非連鎖 7%。→ **選択側に伸びしろは無い。増やすべきは連鎖機会そのもの（配置）**。
- **高/低 併合/手 の比較（254 試合、上位 1/3 vs 下位 1/3、`hi_lo.py`）**: 併合/手 0.655（122 手）vs 0.414（77 手）。25 手目: **最高点 −1.90 vs −1.51（平らな方が高併合）**、**露出率 0.397 vs 0.321**、駒数 12.3 vs 14.5、盤上の型数 8.7 vs 9.6、同型間距離は同じ（2.44 vs 2.43）。→ 平坦さと露出が構造的な差（駒数・型数は結果側でもある）。
- **v754（hash `9a1013eedf1e`、`selfplay_root/strategy_v754.py`、v748 ベース）**: `height_penalty = landing_y × 19.20 × height_mult` の直後に、**駒数 ≤26 の非併合手（締切安全・非 death_spiral・T15 不在・margin≥1.0）で `height_penalty *= 2.5`（tag `EARLY_FLATNESS`）**。オフライン（対 v736）: changed 3793/11155（v748 3766）、DIRECT 損失 0、newcross 0、risk_top −0.217（v748 −0.144 より低い＝より低く置く）。局所同時 P/Q（A=v748 / B=v754、FPS=30、ports 20340/19100, 20360/19120）を 06:37 起動（~08:00）。見る指標: 併合/手、25 手目の最高点、露出率。

## 2026-08-29 06:0x JST — ユーザー質問「ロシア率 9% の頃の戦略は採用されているか」→ 実測: その期間は v736/v748 の A/B 期間そのもの、両方とも現役（v748 が現 root）

- **建国率の履歴（`tmp/history/russia_creation_history.tsv` 93 件、ダッシュボードと同じ 100 試合窓）**: 最大は **8%（8/100、game#46452 終端 = 2026-08-28 16:07 JST）**。9% の窓は履歴上には無い（私の実測は最大 8%。表示のタイミング差か窓の起点差の可能性）。直後の 100 試合は 3%、現在（game#46534–46633）も 3%。
- **その窓（game#46353–46452 = 08-28 08:0x–16:07）は v736(A) vs v748(B) の長期 A/B 期間**。窓内の建国 8 件は **v736 5 件 / v748 3 件**（A/B 記録の T15 到達を時刻照合）。単独の優れた戦略ではなく、両腕が交互に走っていた期間の上振れ。
- **正面比較（各 100 試合）**: v736 建国 6% vs v748 5%（差 1 件＝統計的に同等）。v748 は score +194（p 0.041）・併合/手 +0.038 で採用。**両方とも現役**: v748 = 現 root `897803192d9f`（08-28 23:32 採用）、v736 = revert point `253cc67e0c1b`。→ 失われた戦略は無い。
- **8% は上振れ**: 基準率 4.5% なら 100 試合で 8 件以上が出る確率は 8.7%。現在の 3% も同じ分布内。容量モデル（`手数 = 42/(1−併合/手)`）では建国率は手数の増加関数なので、併合/手 が高い v748 の方が長期的には建国率も高くなる見込み。
- **発見した軽微な汚染**: 現行 A/B（v748 vs v752）の idx=0 は A/B 開始前に始まっていた試合で、hash が v736（253cc67e0c1b）のまま arm=A として記録されている（tainted 判定は history_hash と一致するため素通り）。**k=50 判定時に idx=0 を除外する**。他 83 件は正しい（A: v748 41、B: v752 42）。

## 2026-08-29 05:2x-05:5x JST — **ソ連建国の必要条件を定量化: 併合/手 0.83（現状 0.559）** / 評価指標を score → 併合/手 へ

- **容量モデル（A/B 履歴 710 試合、`tmp/history/ab_*_games.jsonl` ＋ 進行中）**: 1 手ごとに盤面のピースは (1 − 併合/手) 増え、`C = 手数 × (1 − 併合/手)` は **平均 42.0（sd 4.1、p25 39 / p75 45）** で戦略に依らない（corr(C, pc20)=+0.01、corr(C, pc40)=+0.07、高 C 群はむしろ併合率が低い＝偶然のパッキング差）。→ **手数 = 42/(1 − 併合/手)**。
- **到達距離**: 併合/手 0.50→84 手、0.55→93、0.60→105、0.65→120、0.70→140、0.75→168、0.80→210、0.85→280。**250 手（ソ連建国）に必要な併合/手 = 0.832**、200 手 = 0.790、150 手 = 0.720。実測分布は p50 0.522、p90 0.650、p95 0.683、**最高 0.799**（710 試合中 200 手以上はゼロ、最長 199 手）。型別: T15 到達 19 試合は 併合/手 0.719・146 手、T14 191 試合は 0.595・108 手、≤T13 500 試合は 0.486・85 手。
- **含意**: 現 root v748 の 0.559 から 0.83 まで +0.27。v748 の改善幅は +0.038（対 v736）なので **v748 級の改善が約 7 回**必要。序盤のピース数（pc20/pc40）は併合/手を経由してのみ効く中間指標で、最終指標ではない。今後の候補は「併合/手をどれだけ上げるか」で設計・評価する。
- **評価指標の変更（次の A/B から）**: 完了した v736 vs v748（各 100）で同一データを比較 — score: 差 +194、SE 99、**z 1.97**、CV 43%／**併合/手: 差 +0.038、SE 0.013、z 2.82、CV 20%**／手数: +7.4、z 2.44。→ **主要指標を 併合/手 に変更**。必要 n（80% 検出力、両側）: 効果 +0.02 で各腕 412、+0.03 で 183、+0.05 で 66。**実行中の v752 vs v748 は事前登録どおり score（k=50）で判定し、併合/手 は併記**（途中で規則を変えない）。次の A/B から新規則: 主要=併合/手、各腕 60（効果 +0.05 相当）で 1 次判定、境界なら 120 まで延長、ガードレールは T15 到達率と score の符号。
- **実戦 A/B（A=v748 / B=v752）05:29**: games=78（各 39）、score A 1786 / B 1905、k=18 mean(B−A)=+106（SE 175）、T14+ 17 vs 13、T15 1 vs 1 → 継続。04:13 の tick は cron 未発火（05:29 に再開）。
...
- 参考: 新 root v748 の A 腕平均 1959（前 A/B での v748 は 1837）— 水準は維持。
- 次 tick（03:13）: status（k≈11）。

## 2026-08-29 01:1x-01:3x JST — v752 vs v748 長期 A/B k=5 +262（継続）

- **実戦長期 A/B（A=v748 / B=v752）01:29**: games=24（各 12）、A 1800（med 1563、101.3 手）/ B 1997（med 1906、104.6 手）、k=5 mean(B−A)=+262（SE 444）、UCB90 +831 → 害停止なし（k≥6 でも UCB90>0 見込み）。T14+ 6 vs 3、T15 1 vs 0、3000 点超 2 vs 2。decide_exception 増なし。
- 次 tick（02:13）: status（k≈8）。
...
- **v752 vs v753 局所再現（`v753_p2`/`v753_q2`、27/30）**: P2 −126（SE 155）、Q2 −189（SE 136、B=v752）→ v753 ≈ +32。初回（−352）とプール ≈ −160 → v753（EV 再順位付け）が v752 を上回る証拠は無い。EV の重み（250）や範囲（top-12）は要調整だが、局所では判別できないので v752 の実戦結果を待つ。
- 次 tick（20:13）: status（k≈39）。

## 2026-08-28 18:1x-18:3x JST — 新ランナー下で実戦は正常（k=33 +267 p 0.033）/ v753 オフライン OK だが局所 P/Q は割れ（P +168 / Q −858）

- **実戦長期 A/B（v748）18:29**: games=133（67/66）、A 1626（med 1492、90.6 手）/ B 1894（med 1744、100.2 手）、k=33 **mean(B−A)=+267（SE 117、sign-flip p 0.033）** → 継続（k=50 判定、~08:30）。新ランナー（17:33 反映）下で idx 121–132 が正常に記録、decide_exception 増なし、Traceback は 17:33 以降 0（ログ内の 37 件は過去の regression rollback validation のもの）。
- **v753 オフライン（対 v736）**: changed 4558/11155（40.9%、v752 4259）、DIRECT 損失 0、newcross 116（v752 と同じ終盤併合）、risk_top −0.123（v752 −0.096 より低い）。
- **v752 vs v753 局所 P/Q（FPS=30、28/27）**: P（B=v753）+168（SE 175）、Q（A=v753）**−858（SE 109、p 0.03、raw v753 1441 vs v752 2310）** → 効果 ≈ −345、腕文字 ≈ +513。instance 間の乖離が再び極端（v752 が Q で 2310 と異常に高い）。局所の go/no-go には使わない方針どおり、再現 `v753_p2`/`v753_q2` を回して傾向だけ見る。実戦の順番は v752（v748 判定後）→ v753。

## 2026-08-28 17:4x-17:5x JST — 1時間以降のポッドキャストチャプターを修正し、8/26・8/27公開動画へ反映
...
- **コード修正**: `games/soviet_now` commit `5b06dd40f`。`format_youtube_timestamp` を追加し、1時間未満は `M:SS`、1時間以上は `H:MM:SS` で生成。動画を作り直さず説明欄だけ再生成する `--description-only` も追加。新規 `tests/test_podcast_video_chapters.py` 4件、タイトル類似テスト4件、既存 `test_podcast_build.sh`、py_compile、diff checkが成功。
- **過去回の特定と反映**: 公開済み成果物を全走査し、同じ誤りは8/26 `L2mdR3EFpW8` の `64:50` / `69:41` のみ。8/26を `1:04:50` / `1:09:41`、8/27 `RuFN4hJhpi4` を `1:01:45`〜`1:51:20` に再生成し、現在タイトル一致ガード付きでYouTube説明欄だけ更新した（動画再アップロードなし）。
- **公開側の実測**: YouTube API再取得で両説明欄の新形式を確認。Chromeの公開視聴ページでも8/27は `1:01:45`〜`1:51:20` が各 `t=3705s`〜`t=6680s` のリンクになり、チャプターパネルも表示。8/26は `1:04:50` / `1:09:41` が `t=3890s` / `t=4181s` のリンクになった。
...
- **実測**: decide 1 手 v752 0.001 s → v753 0.005 s（最大 0.015 s）。該当試合 40 手の再生で 6 手（15%）が変わる。オフライン検査（対 v752、`scratchpad/v737/screen_v753.txt`）は背景実行中。
- **局所同時 P/Q（A=v752 / B=v753、FPS=30、`v753_p`/`v753_q`、各 2 slots × 15、ports 20260/19020, 20280/19040）**: 17:38 起動（~19:00）。見る指標: 併合/手、pieces@40、手数、T14+/T15。
- 実戦: 新ランナー下の初戦 idx 121（B）進行中、decide 例外増なし。次 tick で記録を確認。
...
- 実施: 走行中の自己対戦が無いことを確認し、全 run の `slot-*` と `seedtest*`／`aa_smoke` を削除 → scratchpad 12 GiB → **941 MB**（自己対戦データ 417 MB＝試合ログのみ残置）。
- 運用ルール（メモリにも記録）: **run 終了後に `slot-*` を必ず削除**、無価値な run はディレクトリごと削除、scratchpad が 2 GB を超えたら古い `games/` も整理。

## 2026-08-28 16:1x-16:3x JST — 長期 A/B（v748）k=26 +276（p 0.051）、天井差も縮小 / v751a（T7/T8 座席除外）は局所で v748 より悪い → 不採用

- **実戦長期 A/B（v748）16:29**: games=106（各 53）、A 1621（med 1439、90.3 手）/ B 1906（med 1742、100.2 手）、k=26 **mean(B−A)=+276（SE 132、sign-flip p 0.051）**、UCB90 +445 → 継続（k=50 判定）。天井: T14+ 25 vs 16、**T15 3 vs 5、3000 点超 3 vs 4**（14:29 の 1 vs 4 から縮小）。
- **v751a vs v748（局所 FPS=30、29/29）**: P −238（SE 238）、Q +278（SE 184、B=v748）→ v751a ≈ −258。raw 1640 vs 1908、手数 91.9 vs 99.7、T14+ 12 vs 14、T15 1 vs 2 → **T7/T8 の座席を外すと悪化**（天井も上がらない）。不採用。座席レーンは T1–T8 のままが正解。
- 次 tick（17:13）: status（k≈29）。局所は待機。

## 2026-08-28 15:1x-15:4x JST — 長期 A/B（v748）k=23 +241（p 0.112）/ 天井低下仮説に v751a（T7/T8 を座席対象外）を局所で機構検証

- **実戦長期 A/B（v748）15:29**: games=94（各 47）、A 1621（med 1419、90.3 手）/ B 1851（med 1700、98.9 手）、k=23 mean(B−A)=+241（SE 142、p 0.112）、UCB90 +423 → 継続。T14+ 到達 B 21 vs A 14、**T15 到達 B 1 vs A 4、3000 点超 1 vs 3**（天井ガードレール要監視）。
- **v751a（hash `ed7473094dc3`、`selfplay_root/strategy_v751a.py`）= v748 の座席から T7/T8 を除外**（T1–T6 のみ）。オフライン（対 v736）: changed 3540（v748 3766）、DIRECT 損失 0、newcross 0、risk_top −0.166。局所同時 P/Q（A=v748 / B=v751a、FPS=30、`v751a_p`/`v751a_q`、ports 20220/18980, 20240/19000）を 15:34 起動（~17:00）。見る指標: T14+/T15 到達率、3000 点超、手数（天井が回復するか）。
- 次 tick（16:13）: 実戦 status（k≈26）、v751a 途中。

## 2026-08-28 14:1x-14:3x JST — 長期 A/B（v748）k=19 +284（p 0.116）/ look 19 は未採用・ガードレール「ロシア到達 B 1 < A 4」

- **実戦長期 A/B（v748）14:29**: games=79（39/40）、A 1630（med 1390、p25 1086、90.7 手）/ B 1913（med 1812、p25 1427、101.2 手）、k=19 **mean(B−A)=+284（SE 169、p 0.116）**、UCB90 +500。`ab_decide` look 19: 未採用（p≥α/2、MDE 412 > 284）＋ガードレール **T15 到達 B=1 < A=4**。長期事前登録どおり k=50 まで継続。
- **到達型の分布（各 40）**: A max_type {12:4, 13:23, 14:9, 15:4}、B {13:22, 14:17, 15:1}。B は T12 止まりが無く T14 到達が多い（18 vs 13）が **T15（ロシア）は 1 vs 4**、3000 点超も 1 vs 3。→ v748 は床（中央値・下位）を上げるが天井（ロシア到達・大連鎖）を下げている可能性。ソ連建国には天井側が必須なので、k=50 の判定では **T15 到達率をガードレールとして扱う**（B の T15 率が A の半分未満なら採用しない）。
- 設計メモ: 座席レーンは T1–T8 を高型側の反対に集めるため、終盤の T13/T14 形成域（高型側）に中型が供給されにくい可能性。次候補では T7/T8 の座席を高型側寄りに（または T9+ の連鎖レーンへ供給）する変種を検討。
- 次 tick（15:13）: status（k≈22）。

## 2026-08-28 13:1x-13:3x JST — 長期 A/B（v748）k=16 +241（継続）

- **実戦長期 A/B（v748）13:29**: games=65（33/32）、A 1699（med 1419、p25 1207、92.9 手）/ B 1950（med 1853、p25 1427、102.7 手）、k=16 mean(B−A)=+241（SE 197、p 0.25）、UCB90 +493 → 継続。decide_exception 増なし、改善候補なし、VM load 8–9。
- 次 tick（14:13）: status（k≈19）。k=50 は翌 08:30 頃。局所は待機。

## 2026-08-28 12:1x-12:3x JST — 長期 A/B（v748）k=13 +286 / v750 再現最終は ≈ −28（プール ≈ +220、不確か）

- **実戦長期 A/B（v748）12:29**: games=52（各 26）、A 1667（med 1573、92.8 手）/ B 1953（med 1976、103.2 手）、k=13 **mean(B−A)=+286（SE 198、p 0.18）**、UCB90 +539 → 継続（判定 k=50）。前回 run（03:30–07:30、k=13 で −160）と合わせた 26 ブロック平均 ≈ +63。今回 run は B の中央値・下位四分位・手数すべて上。
- **v750 vs v748 再現最終（`v750_p2`/`v750_q2`、各 30）**: P2 −22（SE 246）、Q2 +34（SE 361、B=v748）→ v750 ≈ −28 over v748。初回 +471 とプール ≈ +220（SE ≈200）で不確か。v750 の優先度は保留（v748 の長期判定後）。局所スロットは空けておく。
- 次 tick（13:13）: 実戦 status（k≈16）。

## 2026-08-28 11:1x-11:3x JST — 長期 A/B（v748）k=9 +115 / v750 再現は +143（プール ≈ +300 over v748、局所）

- **実戦長期 A/B（v748）11:29**: games=39（19/20）、A 1735（med 1570、p25 1086）/ B 1827（med 1853、p25 1569）、k=9 mean(B−A)=+115（SE 246）、UCB90 +430 → 継続。B は下位四分位が高く「悪い試合が減る」形。
- **v750 vs v748 再現（`v750_p2`/`v750_q2`、FPS=30、29/28）**: P2 +102（SE 254）、Q2 −184（SE 349、B=v748）→ v750 ≈ +143 over v748。初回 +471 とプール **≈ +300**（局所）。機構指標は前回逆方向だったので、実戦候補としての優先度は v748 の長期判定後に決める（v748 が正なら v750 を次の長期 A/B に）。
- 次 tick（12:13）: 実戦 status（k≈12）、v750 再現最終。

## 2026-08-28 10:1x-10:4x JST — 長期 A/B（v748）k=6 +68（害停止なし）/ v750 は局所で v748 に +469 だが機構指標は逆方向 → 再現で確認、実戦は v748 継続

- **実戦長期 A/B（v748）10:29**: games=26（各 13）、A 1662（med 1570、92.6 手）/ B 1777（med 1855、101.1 手）、k=6 mean(B−A)=+68（SE 331）、UCB90 +492 → 害停止なし、継続（判定は k=50）。
- **v750 vs v748（局所 FPS=30、`v750_p`/`v750_q`、28/28）**: P（B=v750）+651（SE 503）、Q（A=v750）−287（SE 222、B=v748）→ v750 ≈ +469 over v748、raw 2066 vs 1640、手数 106.8 vs 92.6。**しかし機構指標は狙いと逆**: EXPOSURE_KEEP 発火 24.8%/手なのに pc40 19.9 vs 18.8（多い）、露出率@40 0.24 vs 0.27（低い）。→ 効果が本物でも想定した機構ではなく、n=28 のノイズの可能性も高い。実戦 A/B は切り替えず v748 の長期を継続。v750 は VM `tmp/manual_challenge/strategy_a856cc1f7b07.py` に待機（validator OK）。再現 `v750_p2`/`v750_q2`（vs v748、FPS=30）を続けて回す。
- 次 tick（11:13）: 実戦 status、v750 再現途中。

## 2026-08-28 09:1x-09:4x JST — 長期 A/B（v748）k=3 +518（観察）/ 機構探索 v750（露出保持の一般化）を局所 FPS=30 で v748 と比較

- **実戦長期 A/B（v748）09:29**: games=12（各 6）、A 1411 / B 1928、k=3 mean(B−A)=+518（SE 288）→ 観察（害停止判定は k≥6、~10:30）。VM load 9–11（soren91 作業）。
- **v750（hash `a856cc1f7b07`、`selfplay_root/strategy_v750.py`）= v748 ＋ EXPOSURE_KEEP**: 非併合手で露出した T3–9 の真上に載る候補を一般に減点（覆う個数 ×120、最大 2 個分。v743 の「唯一の露出個体 −180」は併存）。オフライン（対 v736）: changed 4243/11155（38%）、DIRECT 損失 0、newcross 0、risk_top −0.158（v748 は 3766 / −0.144）。局所同時 P/Q（**A=v748 / B=v750**、FPS=30・internal 576×324、`v750_p`/`v750_q`、各 2 slots × 15、ports 20140/18900, 20160/18920）を 09:34 起動（~11:00）。見るのは機構指標（pieces@40・手数・露出率）で、go/no-go は実戦。
- 次 tick（10:13）: 実戦 status（k≈6 の害停止判定）、v750 途中。

## 2026-08-28 08:1x-08:4x JST — FPS=30 でも局所効果は消えず / 実戦データで圧縮→生存の勾配を確認 → v748 を長期事前登録で実戦再開

- **局所 FPS=30 検証（`v748_fps30_p`/`_q`、07:31–08:45、30/28）**: P +489（SE 281）、Q −503（SE 261、B=v736）→ **v748 ≈ +496**。描画 FPS 制限は乖離の原因ではない（局所は FPS に関係なく楽観）。局所 v748 の B 腕で T15（ロシア）4/16 試合、merges/turn 0.59。
- **実戦の観測的勾配（v736、A/B 履歴 187 試合、試合別指標）**: corr(pieces@40, turns) = **−0.41**、corr(pieces@40, score) = −0.42、corr(pieces@20, turns) = −0.24。三分位: pc40 16.7 → 100.6 手・1894 点、21.1 → 91.3・1631、25.6 → 81.4・1285（≈2.1 手/個）。→ 実戦でも中盤の圧縮は生存に変換される。v748 の実戦圧縮は −1.45 個（局所 −2.8）→ 期待 +3 手 ≈ +80 点。前回実戦 −160±205 はこれと矛盾しない（検出限界 ≈+150 未満）。
- **方針**: 実戦 A/B の検出力（k=13 で MDE ≈ 300）では +100 級の改善は判定できない。実戦枠が空いているので **v748 を k=50（各 100）まで回す長期 A/B を 08:31 に再開**（再開の事前登録は上記 preamble のとおり。害停止は維持）。局所は FPS=30 を標準にするが go/no-go には使わない。
- 迷子 state（idx 54 B 990）を `ab_20260828_073043_games.jsonl` に統合し削除済み（再開前に確認）。
- 次 tick 以降: 各 tick で status のみ（害停止 k≥6 は ~10:30）。k=50 は翌 08:30 頃。
...
- **実戦 A/B（v748）06:29**: games=40（各 20）、A 1752（med 1686）/ B 1692（med 1554）、k=10 **mean(B−A)=−60（SE 250）、UCB90 +260** → CONTINUE（k≥12 で UCB90<150 なら無益停止）。k=6 の +266 から後退。
- **実戦 B 腕の機構（20 試合、1700 手）**: SEAT_LANE 発火 31.0%（局所 30.6%）、pieces@20 11.0 vs A 12.05（局所 10.3 vs 11.5）、pieces@40 18.85 vs 20.3（局所 18.4 vs 21.2）、併合/手 0.52 vs 0.55、**手数 94.6 vs 94.5（局所 101.5 vs 94.8）**。→ 序盤・中盤の圧縮は実戦でも起きているが生存に変換されていない（40 手以降で追いつかれる）。
- **試合終了条件の比較（`endcmp.py`、実戦 14 vs 局所 90）**: 手数 90.6 vs 91.4、最終ピース 42.6 vs 42.4、最終余裕 −0.54 vs −0.51、超過状態の手数 4.7 vs 6.3、最終 max_y 3.16 vs 3.21、settle 0.93/0.91s → 終了条件は同一。実戦でだけ v748 の手数が伸びない理由は n=20 では特定できず（look 19 まで観察）。
- **v748s（座席加点 ±350、d6c1036a37ef）局所 P/Q（30/29）**: P −2（SE 201）、Q で +433（SE 544）→ 効果 ≈ +216。v748（+273）と同程度で加点に比例しない → 座席方針の強化余地は小さい。
- 次 tick（07:13）: 実戦 status（k≈13、無益判定）。

## 2026-08-28 05:1x-05:3x JST — v748 実戦 k=6 +266（害停止なし）/ 局所 3 組目最終 +357（3 run プール ≈ +273）

- **実戦 A/B（v748）05:29**: games=27（13/14）、A 1668（med 1527）/ B 1700（med 1698）、k=6 mean(B−A)=+266（SE 247、p 0.41）、UCB90 +582 → CONTINUE。v741（k=6 で −871）・v744（+122）と比べ最も良い立ち上がり。
- **局所 3 組目最終（`v748_p3`/`v748_q3`、各 30）**: P3 +249（SE 163）、Q3 −465（SE 219、B=v736）→ 効果 ≈ +357、腕文字 ≈ −108。3 run: +144 / +318 / +357 → **プール ≈ +273（SE ≈ 120）**。
- 次: 空きスロットで座席加点 ±350 の変種 `v748s`（機構が加点に比例するか）を P/Q。実戦は 06:13 以降も status のみ、09:30 頃 look 19。

## 2026-08-28 04:1x-04:3x JST — v748 実戦 k=3 +58（観察）/ 局所 3 組目は A 文字側でも正（P3 +85 / Q3 +533）→ 腕文字の偏りは偶然、3 run プール ≈ +257

- **実戦 A/B（v748）04:29**: games=14（各 7）、A 1429 / B 1512、k=3 mean(B−A)=+58（SE 262）→ CONTINUE（k<6）。decide_exception 増加なし。
- **局所 3 組目（`v748_p3`/`v748_q3`、03:37–、29/28 試合）**: P3（B=v748）+85（SE 73、raw 1597 vs 1381、手数 95 vs 82）、Q3（A=v748）mean(B−A)=−533（SE 245、raw v748 1948 vs v736 1457、手数 101 vs 87）→ 効果 ≈ +309、腕文字 ≈ −224（run1/2 は +432/+285 だったので符号反転＝偶然）。**3 run プール効果 ≈ +257**（run1 +144、run2 +318、run3 +309）。手数は 6 instance 全てで v748 が長い。
- 次 tick（05:13）: 実戦 k≥6 の害停止判定、P3/Q3 最終。

## 2026-08-28 03:1x-03:5x JST — v748 再現でも正（P2 +802 p 0.03）→ 4 instance プール +270（z 1.84）→ 実戦 A/B 開始（03:30）

- **v748 再現（`v748_p2`/`v748_q2`、02:35–03:45、各 30）**: P2（B=v748）mean(B−A)=+802（SE 209、p 0.029、raw 2531 vs 1747、手数 116 vs 97）、Q2（A=v748）+167（SE 190、raw v748 1590 vs v736 1717）→ 効果 ≈ +318。初回（+144）とプール **≈ +231**。
- **4 instance プール（60/60）**: v748 1965（sd 826、med 1867）vs v736 1696（sd 777、med 1553）→ **raw +270（SE 146、z 1.84）**、手数 101.5 vs 94.8（+6.7）、20 手目ピース 10.3 vs 11.5、40 手目 18.4 vs 21.2。**注意**: B 文字側の v748 が一貫して大きく正（+576/+802）で A 文字側は負（−288/−167）。腕文字で挙動が変わる理由は無いはずで、instance ごとの偶然と見るが要監視。
- **実戦 A/B 開始 03:30:29**: `tmp/manual_challenge/strategy_897803192d9f.py`（validator OK）、`ab_ctl.sh start … ABBA` → state a=253cc67e0c1b b=897803192d9f、REGRESSION_DISABLED=1。実戦ペース ~13 試合/h → k=6 は ~05:30、look 19 は ~09:30。fixture テスト（SEAT_LANE）は未作成、採用時に作る。
- 次 tick（04:13）: 実戦 status（早期は観察）。05:13 以降で k≥6 の害停止判定。

## 2026-08-28 02:1x-02:4x JST — v744 実戦は無益停止（finish A）/ v748 SEAT_LANES 初回: 判定不能だが盤面圧縮の機構指標が動く → 再現 run

- **実戦 A/B（v744 aa37f7327d1f、18:43–02:30、109 試合）最終**: k=27（55/54）**mean(B−A)=−39（SE 126）、UCB90 122 < 150 → REJECT_FUTILE**。`finish A` 実行、root 253cc67e0c1b、REGRESSION_DISABLED=0、SOREN_AB_ALT_STRATEGY 空、記録 `tmp/history/ab_20260828_023015_{state,games,report}`（迷子 idx 109 B 1088 を統合、110 件）。バナー停止（`{"active":false}`）。v744 の実戦効果は ≈0（局所 +190）。害ではないが採用理由もない。
- **v748 SEAT_LANES 局所同時 P/Q 初回（01:35–02:35、各 30）**: P（B=v748）mean(B−A)=+576（SE 378、raw 2142 vs 1490、手数 107 vs 88）、Q（A=v748）+288（SE 213、raw v748 1501 vs v736 1818）→ 効果 ≈ +144（SE ≈217）、腕文字 ≈ +432（v748 の分散大: sd 838–843）。**機構指標（30 vs 30 試合）: SEAT_LANE 発火 30.6%/手、20 手目ピース 10.0 vs 11.5、40 手目 18.9 vs 21.6、手数 99.2 vs 92.9、raw 1840 vs 1661（+179）**。序盤の盤面圧縮という狙いは v741–v747 で唯一はっきり動いた。再現 `v748_p2`/`v748_q2`（各 2 slots × 15、ports 19980/18740, 20000/18760）を 02:35 起動（~04:00）。判定: 2 run プール効果 ≥ +250（SE ≈150）なら実戦 A/B（`tmp/manual_challenge/strategy_897803192d9f.py` は未配置、開始前に scp＋validator）。
- 次 tick（03:13）: v748 再現途中。04:13 で最終判定。

## 2026-08-28 02:1x-02:3x JST — メタ出力漏れが再発。原因は「出力ガードが docich へ委譲され、正典側に推論タグ除去が無い」こと

- **再発 (監視が検知)**: 02:14:37 の読み上げ本文（`spoken_history/20260828_021437_1490_main.txt`）に
  **英語の作業メモ 5 段落**が混入（`The work indicator script is failing due to sandbox network restrictions...`）。
  `<think>` タグも `---` 区切りも無いため、既存 2 段のガードでは落ちなかった。
- **根本原因（実測）**: `_ai_guard_model_output` は `.env` の
  **`DOCICH_BIN=/home/ubuntu/docich/bin/docich` 経由で `docich ai-guard` へ委譲**される
  （`lib/ai_generate.sh` の設計どおり／コメントに「正典は docich 側」とある）。
  ところが正典 `src/docich/model_output_guard.py` は **127 行**で `REASONING_*`（推論タグ除去）を
  **持たず**、`games/soviet_now/lib/model_output_guard.py` の **236 行**版にだけある。
  → **soviet_now 側のガード強化は VM では一切効いていなかった**。
  実測: VM で `docich ai-guard` に 02:14 の出力を流すと **rc=0 / 1830 バイトがそのまま素通り**、
  soviet_now 版を通すと len=0（棄却）。
  **これは 2026-08-27 の「思考漏れを直した」という記録の前提を一部崩す**。
  01:17 の配線自体は正しく、`</think>` 形は docich 側でも棄却されるため塞がっていたが、
  「soviet_now 版ガードが効いている」という理解は誤りだった。
- **soviet_now 版をそのまま通す案は却下（実測）**: 実出力 116 本へ当てると
  `WORK_NOTE_RE` が「徹底調査します」「すぐ確認します」という**自然な日本語**に誤爆し、
  正当な返答 2 本（`20260826_175050` / `20260826_185356`）を**全体棄却**した。
- **修正 (`games/soviet_now` commit `0fc0e007f`)**: 誤爆しようのない処理だけをコメント側に持つ。
  - `_comment_strip_reasoning_tags`: 推論タグの対応ペア・孤立開始/閉じタグを除去（全体棄却はしない）。
  - `_comment_strip_nonjapanese_head`: 先頭から続く「日本語比率の低い段落」を落とす。
    **閾値 0.5 は実測に基づく**（実出力 116 本で、正当な返答の先頭段落は最低 **0.838**・中央値 0.946、
    作業メモ段落は最大 **0.19**。谷間が広い）。`===SING===` 等のマーカー段落で停止、
    日本語が残らないときは元テキストを返す。`COMMENT_MIN_JP_RATIO` で調整可。
  - `_comment_guard_japanese_text` を新設し**日本語本文の 2 経路だけ**で使用。
    英訳生成は日本語ゼロが正しいので従来の `_comment_guard_model_text` のまま。
- **検証**:
  - **実 VM 上で docich 委譲を含む本番チェーン**を通し、02:14 の漏れが正しい日本語返答
    （`esu303さん、[レア]読め、そして学べの獲得、おめでとうございます。…`）になることを実測。
  - **誤爆スイープ**: 実出力 116 本に自前ストリッパのみを適用 → **変化 6 本（すべて実際の漏れ）、
    空になったもの 0 本**。6 本すべて before/after を確認し、正しい日本語本文が残ることを目視確認。
    漏れは 116 本中 6 本 ≈ 5% と、思っていたより高頻度だった。
  - テスト 26 → **40 assertion**、ローカル・VM とも PASS。既存 comment 系 4 本 + python 94 pass。
  - VM 反映 SHA256 一致、chat 02:24:17 / kick 02:24:31 / youtube 02:24:41 `reload complete`。
- **ユーザー判断待ち（重要）**:
  1. **ラジオ経路も同じ委譲を使っている**ため、推論タグ除去は同様に効いていない。
     正典 `src/docich/model_output_guard.py` を soviet_now 版へ寄せるかどうか。
     寄せる場合は `WORK_NOTE_RE` の誤爆（「確認します」等）を先に直さないと、
     正当なラジオ原稿・コメントを棄却する副作用が出る。
  2. 正典 docich 側は現状でも `WORK_NOTE_RE` を持つため、**いまも正当な返答を数%棄却している**
     （実測 116 本中 2 本）。棄却は放送事故にはならない（再生成される）が、無駄な生成コスト。

## 2026-08-28 01:1x-01:4x JST — v747 再現で ≈0（棚上げ）/ v744 実戦 k=23 +43 / 全体方針 v748 SEAT_LANES を起動

- **v747 局所再現（`v747_p2`/`v747_q2`、00:30–01:30、各 30）**: P2（B=v747）−425（SE 216、p 0.11）、Q2（A=v747）−67（SE 344）→ 効果 ≈ −179。初回（+174）とプール **≈ 0**（raw v747 1783 vs v736 1811、n=60/60）。終盤併合優先の機構は「見送りは無益」という根拠が正しくても、発生が 0.36 回/試合と稀で局所では測定不能 → **単独候補としては棚上げ**。ランナーフックは repo（`14a0f34e9`）に残し、VM 反映は v747 系を実戦に出すときのみ。
- **実戦 A/B（v744）01:29**: games=94（各 47）、A 1586（med 1419）/ B 1644（med 1545）、k=23 mean(B−A)=+43（SE 130）、UCB90 +210 → CONTINUE（無益停止 UCB90<150 に未達）。look 37（各 74）は ~05:00。
- **総括（v741–v747）**: 局所配置調整は全て効果 ±200 以内（v744 が最大 ≈+190、実戦 ≈0）。SE ≈100 の評価で判別できず、ソ連建国（生存 2.5 倍）に必要な段階的変化ではない。
- **v748 SEAT_LANES（hash `897803192d9f`、`selfplay_root/strategy_v748.py`、v744 ベース）**: 高型（T≥10）の平均 x の符号で高型側 S を決め（無ければ −1）、反対壁から T1/T2 2.8, T3 2.4, T4 2.0, T5 1.55, T6 1.1, T7 0.6, T8 0.15 の座席 x（= −S×offset）を割当。非併合候補に `220×(1−d/1.2)`（d=|x−seat|、±220 で clip）を加点（tag `SEAT_LANE`）。安全条件は既存と同じ（margin≥1.0、非交差、非壁回転、非 death_spiral、T15 不在）。T9+ は既存のアンカー系ロジックに委ねる。オフライン: changed 3766/11155（33.8%）、DIRECT 損失 0、newcross 0、risk_top −0.144。局所同時 P/Q `v748_p`（A=v736/B=v748、ports 19940/18700）/`v748_q`（入替え、19960/18720）を 01:35 起動（~03:00）。判定: 効果 ≥ +300（局所で検出可能な大きさ）なら再現 run → 実戦 A/B。
- 次 tick（02:13）: v748 途中、v744 status。
...
- **機構（コード）**: strategy.py 1326–1352 行「不可侵の安全不変条件」= 非超過候補が 1 つでもあれば超過フラグ付き候補を**併合グレードに関係なく**除外（例外は超過状態かつ危険ピース対象の DIRECT のみ）。DIRECT 併合は 97% 成立して落下ピースは消えるのに、落下時に一瞬跨ぐという理由で捨てている。さらに `strategy_runner.enforce_deadline_safety`（803–2884 行の最終安全弁）が併合を差し戻す（`avoid_merge_result_deadline_safe` / `safe_postcondition_deadline_headroom`）。
- **影響の実測（495 試合、`endgame_guard.py`/`guard_stats.py`）**: ガードが DIRECT ありで SAFE_LANDING を選んだ 385 件のうち **180 件（0.36/試合）は併合せず**: 生存中央値 9 手・3 手以内死亡 17%・3 手後ピース +1.08。ガードが結果安全な DIRECT を取った 694 件: 97% 併合・生存中央値 13 手・死亡 8%。「併合結果が越える」判定の DIRECT を取った 932 件: 生存中央値 7 手・死亡 23% vs 見送り 25 件: 中央値 5 手・死亡 28%・ピース +1.08。→ 見送りは生存を延ばさず盤面を増やすだけ（ユーザー主張どおり）。
- **デッドライン判定の較正（554 試合、`deadline_calib.py`）**: 「越える」とされた落下の 2 手以内ゲームオーバー率: 余裕 <0.5 で 26%（越えない落下でも 9.6%）、**0.5–1.0 で 0.1%**、1.0–2.0 で 0%。超過状態（deadline_crossed）になってからの生存: 中央値 6 手、平均 15 手、42% は 10 手以上。→ 超過フラグは保守的で「超過＝即死」ではない（ユーザー「そもそも超過してなかった」を支持）。
- **v747（hash `c437ab723ed8`、`selfplay_root/strategy_v747.py`、v744 ベース）**: (1) 不変条件で `merge_grade==DIRECT` は超過フラグでも残す。(2) ガード内で結果安全 DIRECT → 安全 NEAR の次に「任意の DIRECT」（`DEADLINE_GUARD_DIRECT_MERGE_CROSSING`、risk_top 最小）を安全着地より優先。(3) ラッパーに desperate モード: 理由に `DEADLINE_GUARD_NO_VALID` があるか超過状態なら、露出同型（relief 最小）へ margin/超過/壁回転フィルタ無しで直落とし（`OPEN_TWIN_MERGE_DESPERATE`）。(4) モジュール属性 `DEADLINE_ALLOW_DIRECT_CROSS = True`。オフライン: changed 2523/11155（22.6%）、DIRECT 損失 0、newcross 116（意図的）、risk_top −0.028。該当試合の再生: t141 −3.0（T7 併合）、t147 1.3（T11 直落とし）、t148 −2.4（T8 直落とし）、t149 −3.0、t150 −1.9。v736 は不変。
- **ランナーフック（`selfplay_root/strategy_runner.py`、原本 `strategy_runner.py.pre_v747`）**: `enforce_deadline_safety` 冒頭で、戦略が `DEADLINE_ALLOW_DIRECT_CROSS=True` を宣言し、理由に `DEADLINE_GUARD_DIRECT_MERGE_CROSSING`（かつ候補が DIRECT）または `OPEN_TWIN_MERGE_DESPERATE` を含むときだけ差し戻さない。宣言の無い戦略には不活性（同一コピー再生 changed 0、該当試合の v736 再生も不変）。**実戦投入時は VM の strategy_runner.py を境界で差し替え＋リポジトリ commit が必要**（A/B の両腕に配るが A には不活性）。
- **局所同時 P/Q**: 23:14 起動、`v747_p`（A=v736/B=v747、ports 19860/18620）/`v747_q`（入替え、19880/18640）、各 2 slots × 15（~00:40）。v746 P/Q（~23:50）と並走（load 6–7）。
- 実戦 A/B（v744）23:10 時点: games=61（31/30）、k=15 mean(B−A)=+40（SE 174）、UCB90 +263 → CONTINUE。
...
- **v744 途中（18:30、P 28 / Q 30 試合）**: P（A=v736/B=v744）mean(B−A)=+443（SE 178、p 0.098）、raw 2049 vs 1790。Q（A=v744/B=v736）mean(B−A)=−310（SE 173、p 0.19）、raw 1748 vs 1441 → **効果 ≈ +377（SE ≈124）、腕文字効果 ≈ +67**。v742 単独 ≈ +100、v743 単独 ≈ 0 なので**相乗効果**（露出を保つ＋露出を使う）。VM に `tmp/manual_challenge/strategy_af427ce3b5c0.py` を待機配置。
- **LLM 候補（A/B ゲート dry-run、`tmp/state/ab_candidate`、16:54 emit、hash `51d22b6b10e2`、base 253cc67e0c1b）**: 変更内容は T12–14 の低位置レーン距離と混雑時の強制ドロップ等（change_log 末尾）。オフライン（修正済み ab737）: changed 754/11155（6.8%）、DIRECT 損失 0、newcross 0、risk_top −0.10。方針どおり Claude 候補（v744）優先。v744 終了後に局所同時 P/Q（`llm2_p`/`llm2_q`）で比較し、v744 を上回らなければ不採用（`ab_candidate_skipped_offline` 相当を履歴へ）。局所コピー `scratchpad/cand2/`。
- **整列度の観測分析（`sortedness.py`、局所 360 試合、25 手目）**: x と type の順位相関 |ρ| は平均 0.23 で、最終手数との相関 0.05、スコア 0.06（三分位 92→94→95 手）。**露出率（上に何も乗らないピースの割合、平均 0.35）は手数 +0.22 / スコア +0.19、ピース数は −0.27**。→ 「整列（座席レーン）」より「露出」が効く指標。座席レーン方針は優先度を下げ、**露出率を直接最大化する評価軸**（落下後の露出ピース数の増減）を v745 の候補とする。
- 次 tick（19:13）: v744 最終判定 → 正なら fixture テスト（`tests/test_v744_exposure.py`）、実戦 A/B 開始、LLM 候補の局所 P/Q 起動。

## 2026-08-27 17:4x-18:1x JST — 共通乱数（CRN）実験は失敗・打ち切り / v744 進行中

- **CRN 実験**: `SOREN_RNG_SEED` の addInitScript で (1) `crypto.getRandomValues`+`Math.random` → (2) さらに `Date`/`Date.now` を固定基準+経過時間に → (3) さらに `performance.now` を起動 20 秒間 500ms 量子化、の 3 段で同一シード 2 試合の next_type 列を比較したが、いずれも先頭から不一致（一致 10–11/100 ≈ 偶然の 1/11）。Unity(IL2CPP/Emscripten) の乱数シードは JS 側の差し替え可能な時計・乱数源から取られていない（wasm 内部のカウンタ等）と判断し **打ち切り**。`selfplay_root/soviet_local.mjs` は原本に復元（試作は `soviet_local.mjs.crn_attempt`）。リポジトリ未変更。→ 局所の検出力は現状のまま（同時 P/Q 60+60 で MDE ≈ +400、SE ≈150）。スロット増（8）で時間当たりの n を増やす以外にない。
- **v744（v743+v742）**: 18:10 時点 P 6 / Q 7 試合。~19:00 に最終。
- **方針の見直し（3 候補の結果を受けて）**: v741（局所 +183／実戦 −528）、v742（≈+100）、v743（≈0）。配置の局所調整は ±100 の域で、ソ連建国（生存手数 2.5 倍）に必要な段階的変化ではない。次は「配置の全体方針」（人間型の整列: 型ごとの座席レーン、v741 のジャンク隅寄せを全型に一般化）を fable 設計で検討し、オフライン指標（20 手目のピース数・双子到達率）で事前に篩ってから局所 P/Q へ。

## 2026-08-27 17:1x-17:3x JST — v743 は効果ゼロ（同時 P/Q 各 30: −69 / +54 → ≈ −8）/ v744（v743+v742）P/Q 起動 / 共通乱数（同一ピース列）実験を開始

- **v743 LAST_EXPOSED_COVER_AVOID 最終（15:39–17:00）**: P（A=v736/B=v743）mean(B−A)=−69（SE 220）、Q（A=v743/B=v736）−54（SE 169）→ **効果 ≈ −8（SE ≈140）**。raw 平均は v743 1598/1374 vs v736 1559/1362。タグは 16.8% の手で発火したのにスコア不変 → 「露出した唯一の同型を覆わない」だけでは併合は増えない（露出を保っても双子が来る前に別の理由で埋まる／代替位置が劣化）。**v743 単独は不採用**。v742 ≈ +100、v743 ≈ 0 で、配置の局所調整では段階的変化に届かないことが 3 候補（v741/742/743）で確認された。
- **v744（v743+v742、af427ce3b5c0）**: 17:32 から同時 P/Q（`v744_p`/`v744_q`、各 2 slots × 15、ports 19660/18420, 19680/18440、~19:00）。期待は v742 相当（≈+100）。
- **共通乱数（CRN）実験**: 局所の検出力律速（60+60 で MDE ≈ +400）を破るには A/B を同一ピース列で対にするのが最善。Unity(Emscripten) の wasm strings に `/dev/urandom`・`random-seed` があり、起動時シードが JS の `crypto.getRandomValues` 経由なら固定できる。`selfplay_root/soviet_local.mjs`（コピー、原本は `.orig`）に `SOREN_RNG_SEED` 環境変数で `crypto.getRandomValues`/`Math.random` を決定的 PRNG に差し替える addInitScript を追加（リポジトリ未反映）。17:32 に同一シード（4242）・両腕 v736・2 slots × 1 試合の `seedtest` を起動 → 2 試合の next_type 列が一致すれば成功（次 tick で判定）。成功なら harness に `--seed-base` を足し、ブロック内で同一シードの A/B 対を組む（分散が大幅に減る見込み）。失敗なら Unity 側の時間ベースシードの可能性 → 断念。
- 実戦: root v736、A/B なし。次 tick（18:13）: seedtest 判定、v744 途中。

## 2026-08-27 16:1x-16:3x JST — v742 run 3 最終: 効果 ≈ +189（腕文字効果 −37）、run 1–3 合算 ≈ +100±100 → 保留 / v743 途中 ≈ +64 / v744（v743+v742）を準備

- **v742 run 3（同時 P/Q、14:34–16:25）**: P（A=v736/B=v742、29+30）mean(B−A)=+152（SE 226、p 0.53）、Q（A=v742/B=v736、29+31）mean(B−A)=−225（SE 212、p 0.33）→ **v742 効果 ≈ (152+225)/2 = +189（SE ≈155）、腕文字効果 ≈ (152−225)/2 = −37**（run 1/2 では +400 側だったので、腕文字効果は run 時間帯の偶然）。run 1–3 の v742 推定（+417、−395、+152、+225）の平均 **≈ +100（SE ≈100）** → 採用基準（≥+150 かつ p<0.1）未達で**保留**。単独で実戦 A/B には出さない。
- **v743 途中（16:30、各 ~14 試合）**: P（B=v743）+9（SE 243）、Q（A=v743）−119（SE 184）→ 効果 ≈ +64。~17:00 に最終。
- **v744（hash `af427ce3b5c0`、`selfplay_root/strategy_v744.py`）**: v743（露出保持 −180）に v742 のラッパー（露出同型・relief≤1.0 の直落とし）を重ねた相補候補。オフライン: changed 2072/11155（18.6%）、DIRECT 損失 0、newcross 0、risk_top −0.10。v743 P/Q 終了後に同時 P/Q で局所評価（~17:05 起動、~18:30）。
- 実戦: root v736、REGRESSION_DISABLED=0、A/B なし。VM load 10–11（soren91 作業）。
- 次 tick（17:13）: v743 最終判定、v744 P/Q 起動、v743 が正なら fixture テスト作成。
...
- **「大ピースの間に小ピースが滑り込んで併合不能」（`scratchpad/wedge.py`）**: 局所 v736 60 試合で同型大ピース（T≥8）の隙間に小ピース（T≤4）が挟まる事象 2.85 回/試合、50% の試合が挟まったまま終了（実戦直近 14 試合 2.3 回/試合、21%）。内訳: 自分の落下で挟めた 17 / 転がり込み 11 / **既存の小ピースの両側に大ピースが（併合で）生まれた 145**。→ 落下制御より「大ピースの隣に居座る小ジャンクを早く消す」が筋。
...
- **実戦 A/A 最終（08:38–14:32、84 試合）**: A 1587（n=42、sd 907、med 1475）/ B 1508（n=42、sd 733、med 1272）、ブロック k=21 **mean(B−A)=−79（SE 193、p 0.69）**、UCB90 +169、LCB90 −326。事前登録: UCB90<0 なし → 偏りの証拠なし。LCB90>−300 は僅かに未達 → 形式上「未決」だが点推定は −79。**実戦 A/B 計測器は使ってよい**（v738/v741 の判定は有効とみなす）。`finish A` 実行、root 253cc67e0c1b、REGRESSION_DISABLED=0、記録 `tmp/history/ab_20260827_143245_*`（84 件）。進行中だった試合の迷子 state は次 tick で統合。
- **v742 局所クロスオーバー**: run 1（A=v736/B=v742）+417（p 0.057）に対し **run 2（A=v742/B=v736、`v742_x2`、13:34–14:35、30+30）は A(v742) 1448 / B(v736) 1833、mean(B−A)=+395（SE 211、p 0.09）= v742 −385**。プール ≈ +16。同一腕 A/A（probe2）では +36 だったのに、v742 の 2 run はどちらも「B 文字側」が +400 → 局所ハーネスに run ごとの時間帯効果か腕文字効果がある疑い（差 800 点は SE から見てノイズでは説明しにくい）。**run 3**: 14:34 から同一時間帯で `v742_x3p`（A=v736/B=v742、2 slots × 30、ports 19540/18300）と `v742_x3q`（A=v742/B=v736、ports 19560/18320）を同時実行（~16:20）。推定: v742 効果 = ((B−A)_p − (B−A)_q)/2、腕文字バイアス = ((B−A)_p + (B−A)_q)/2。
- **解析器の根本原因（`scratchpad/analyzer_diag.py`、発火事例 184 件を解析器で再計算）**: twin の x に落とす候補について、解析器は **98% で landing_hit_id = twin ではない隣のピース（多くは T6–T9）** と予測し merge_grade=NO。実際にはそれらの 60–84% が併合（例: twin (1.58, −2.06) に対し解析器は id31 に y=−0.6 で着地と予測、実際は併合）。= 解析器は一次着地先だけで併合を判定し、**静止時に twin と接触する二次接触を併合として評価していない**。v742 はこれを戦略側で回避しているだけ。
- **解析器アップグレード方針（ユーザー提案 14:4x「解析機のほうもアップグレード検討するべき」に同意）**: analyze_board に env トグル（例 `ANALYZE_BOARD_SECONDARY_CONTACT=1`）で「予測静止位置で接触する同型（二次接触）」を検出し、relief で較正した確率（<0.5: ~0.8、0.5–1.0: ~0.6、>1.0: ~0.2）付きの `NEAR`/`PROBABLE` として候補に載せる。既定 OFF で挙動不変（hash 機構は strategy のみなので `analyzer_modes` に記録されるトグルで管理）。評価は局所ハーネスに腕別 env（`--b-env`）を追加して自己対戦 → 実戦 A/B は `_ab_select_arm` に B 腕 env 付与を追加して実施。設計は fable（自分）で、run 3 の結果を見てから着手。
- **視聴者返答**: 検証結果（T11 平均 2 個 / T12 2.2 個、7 割の試合で 2 個以上残留、隣で生まれる T12 は 17%、対策 v742/v743、正直な見積り）を `enqueue_audio_text … viewer_reply` で 14:33 にキュー投入（`tmp/.comment_queue/comment_announce_*_viewer_reply.txt`）。**再生は未確認**（次 tick で sent/ログを確認）。
- 次 tick（15:13）: 迷子 state 統合、viewer_reply 再生確認、run 3 途中集計、解析器アップグレードの実装着手（analyze_board の候補生成箇所の読解から）。

## 2026-08-27 13:5x JST — 視聴者コメント「ウズベク(T11)・トルクメン(T12)が残りすぎ、寄せて併合すればロシアくらいできたかも」の検証 → 事実。v743 候補の種

- **実戦直近 62 試合の終了盤面（VM game_history）**: T11 平均 1.97 個、T12 平均 2.18 個。同型 ≥2 が残る試合は T11 71%（44/62）、T12 73%（45/62）。T12 ペアは 29/45 で左右に 3.0 以上離れ、27/45 で両方とも埋没。T13 は平均 0.95、T14 0.29。
- **原因（局所 v736 60 試合、`scratchpad/t12_chain.py`）**: T12 は落とせず T11+T11 の併合で生まれる。T12 誕生 249 のうち既存 T12 がある 151 件で、その近く（≤2.2）で生まれたのは 25（17%）だけ、遠くが 126。近くで生まれると 25 手以内に T13 化 44%、遠いと 37%。T11 の落下 465 のうち T12 の近く（≤1.5）は 44%、T11 の近くは 51%。→ 既存の `T12_CHAIN_LANE_GUIDANCE` は弱く、T12 の誕生位置を既存 T12 の隣に誘導できていない。終了時 T11≥2 は 65%、T12≥2 は 70%（実戦と一致）。
- **v742 run 1 の B 腕（29 試合）**: 既存 T12 の近くで生まれた T12 の T13 化 69%（11/16）、終了時 T12≥2 は 63%。露出した T11 ペアへの直落としも v742 の対象。
- **v743 の種（未設計）**: 「T11 を（併合不可のとき）既存 T12 の隣の開いた側に置き、後の T11+T11 併合で生まれる T12 が既存 T12 に接するようにする」= T12 の誕生位置誘導。効果上限の見積り: T12 ペア → T13 で +1 T13/試合、T13 平均 0.95 → T14 が 1 つ増える程度。ロシア(T15)には T14×2 が必要なので単独では届かないが方向は正しい。v742 の判定後に設計（fable）→ 局所クロスオーバー → 実戦 A/B の順。

## 2026-08-27 13:1x-13:4x JST — v742 局所クロスオーバー run 1: +417（p 0.057）/ run 2（入替え）起動 / 実戦 A/A k=17 −214

- **run 1（A=v736 / B=v742 `6dd86219ecd2`、`tmp/selfplay/v742_x1`、12:34–13:35、59 試合＋1 進行中）**: score A 1575（n=30、med 1450、p25 1202、93.8 手）/ B 1915（n=29、med 1633、p25 1389、101.0 手）、ブロック k=14 **mean(B−A)=+417（SE 188、sign-flip p 0.057）**。B 腕で `OPEN_TWIN_MERGE` 発火 87 回（3.1/試合）、実現併合 61%（他の併合なし手 13%、解析器判定の併合 95%）。
- **run 2（入替え A=v742 / B=v736、`tmp/selfplay/v742_x2`、ports 19520/18280）**: 13:34 起動、~14:35。判定は 2 run プール（各 60）: v742−v736 ≥ +150 かつ p<0.1 なら実戦 A/B（A/A 終了後、`bash tools/ab_ctl.sh start tmp/manual_challenge/strategy_6dd86219ecd2.py ABBA`、事前登録は v741 と同じ: k≥6 で UCB90<0 → 害停止、looks 19/37 で採否）。
- **実戦 A/A**: 13:30 時点 games=68、k=17 mean(B−A)=−214（SE 206、p 0.33）。14:30 前後の k=20 で判定（UCB90<0 → 偏りあり / LCB90>−300 → 健全 / それ以外は未決で終了）→ `finish A`。
- 注意: 13:3x に `pgrep -f` が "illegal byte sequence" で失敗し待機ループが即抜けたため、run 1 の最終 1 試合が進行中のまま run 2 を起動（ポート別なので結果には影響なし、数分の CPU 競合のみ）。macOS の pgrep は `LC_ALL=C pgrep` で回避する。
- 次 tick（14:13）: A/A の k=20 判定・finish、run 2 途中集計。

## 2026-08-27 12:1x-12:4x JST — プローブ 379 回で較正: relief が主因・型は無関係 → v742 は relief≤1.0 のみ（hash 6dd86219ecd2）/ 局所クロスオーバー A/B run 1 起動 / 局所 A/A（同一腕）で B 腕バイアスなし

- **較正（probe2 60 試合 + probe 16 試合 = 379 発火、`probe_feat.py`）**: 実現併合 55%。relief（隣接 |dx|≤1.2 の最大上端 − 双子上端）別: <0.5 で 114/144=79%、0.5–1.0 で 70/119=59%、>1.0 で 23/116=20%。relief を固定すると型差は消える（relief<0.5: T1–2 79% / T3–5 85% / T6+ 75%）→ 型フィルタは撤去。壁際は 45% vs 非壁際 56%（未フィルタ）。
- **v742 最終形（hash `6dd86219ecd2`、`selfplay_root/strategy_v742.py`、VM `tmp/manual_challenge/strategy_6dd86219ecd2.py`。旧 7ead80b12454 は破棄）**: 条件 = DIRECT/NEAR 候補なし ∧ 露出した同型あり ∧ relief ≤ 1.0 ∧ deadline 余裕 ≥2.0 ∧ 真上候補が非交差・非壁回転・|Δx|≤0.35 → x=双子の x、reason `OPEN_TWIN_MERGE`。オフライン: changed 427/11155（3.8%）、DIRECT 損失 0、newcross 0、risk_top +0.22。期待 ≈3.7 発火/試合 × (70%−12%) ≈ +2 併合/試合。
- **局所 A/A（probe2 は両腕同一 512fe504863c、ABBA stagger、30+30）**: A 1806 / B 1860、ブロック k=15 mean(B−A)=+36（SE 178、p 0.84）→ **局所ハーネスに B 腕バイアスなし**（08:5x の「≈+130」推定は run 間分散で確定）。参考: probe 両腕の平均 1833・手数 99 は同日の v736 局所値（~1650・~91 手）より高いが run 間比較なので参考値。
- **局所クロスオーバー A/B run 1**: 12:34 起動、A=v736 / B=v742、4 slots × 15、ABBA stagger、`tmp/selfplay/v742_x1`（ports 19500/18260、~13:35）。終了後に run 2（入替え A=v742 / B=v736、`v742_x2`）。判定は 2 run のプール（各 60）で B−A 平均 ≥ +150 かつ sign-flip p<0.1 を目安に実戦 A/B へ。
- **実戦 A/A**: 12:35 時点 games=54、k=13 mean(B−A)=−87（SE 256）。k=20（~14:30）で判定。
- 次 tick（13:13）: x1 途中集計、A/A status。

## 2026-08-27 11:1x-11:5x JST — 実戦 A/A は k=10 で未決（外れ値 1 試合、偏りの証拠なし）→ k=20 へ延長 / 解析器の併合判定が保守的すぎる領域を実測（プローブ落下 53% 併合）→ 候補 v742 OPEN_TWIN_MERGE を作成

- **実戦 A/A 11:30 時点**: games=41、A 1569（n=21、sd 983）/ B 1359（n=20、sd 528）、ブロック k=10 mean(B−A)=−228（SE 224、p 0.40）、UCB90 +58、LCB90 ≈ −515 → 事前登録の「偏り確定」「健全」どちらにも該当せず未決。腕間で analyzer_modes（merge_top 2 / vertical_lane 1 / wall_clamp 1）・settle_req 3・settle 待ち（0.95 / 0.90s）は同一、併合/手 0.49 / 0.49、20 手目ピース数は B 11.6 vs A 12.7。A の外れ値 idx35（5077 点・199 手）を除くと A ≈ 1390 vs B 1359。**偏りの証拠なし**だが MDE 未達なので **k=20（~14:30）まで延長**。判定規則（事前登録）: k=20 で UCB90<0 → 偏りあり、LCB90>−300 → 健全、それ以外は未決のまま終了。終了は `finish A "A/A instrument check"`。
- **プローブ v742probe（局所 A=v736 / B=probe、16+16、10:48–11:25）**: B 腕で `OPEN_TWIN_PROBE` 発火 86 回（5.4/試合）、**実現併合 53%**（対照: 他の併合なし手 11%、解析器が DIRECT/NEAR とした手 97%）。スコアは B 1804 vs A 1758（+46、SE 285、n=16、判定不能）。**幾何で成否が強く分かれる**（`probe_feat.py`）: 隣接（|dx|≤1.2）の最大上端 − 双子上端 = relief が <0.5 で 29/34=85%、0.5–1.0 で 14/23=61%、>1.0 で 3/29=10%。型別: T1 21%、T2 33%、T3 56%、T4 86%、T6 67%、T8 83%、T11 83%。壁際/非壁際は差なし（50/54%）。
- **候補 v742（hash `7ead80b12454`、`selfplay_root/strategy_v742.py`、VM `tmp/manual_challenge/strategy_7ead80b12454.py` に待機）**: probe と同じラッパー構造（`decide`→`_decide_base`）で、発火条件に **relief ≤ 1.0 かつ next_type ≥ 3** を追加、reason `OPEN_TWIN_MERGE`。オフライン再生: changed 336/11155（3.0%）、DIRECT 損失 0、newcross 0、risk_top +0.22。期待値: 発火 ≈3/試合 × 併合率 ≈75%（対照 11%）≈ +2 併合/試合（序盤・中盤の盤面圧縮）。
- **較正データ収集中**: 11:31 から両腕 probe の局所 60 試合（`tmp/selfplay/v742_probe2`、ports 19480/18240、~12:35）→ ~320 発火で relief/型の閾値を再較正。
- **評価計画**: probe2 終了後に局所クロスオーバー A/B（A=v736/B=v742 と入替え、各 60 試合、~2 h）。正なら実戦 A/B（A/A 終了後）。v741 は棚上げのまま。
- 次 tick（12:13）: probe2 の集計と閾値較正、v742 局所 A/B 起動。A/A は status のみ。

## 2026-08-27 10:1x-10:5x JST — 実戦 A/A k=7 で −72（計測器に偏りなし傾向）/ 序盤の併合漏れ分析 → 「露出した同型に直落とし」プローブを局所で計測開始

- **実戦 A/A 10:50 時点**: games=30、ブロック k=7 mean(B−A)=−72（SE 206、p 0.79）。計測器の B 腕バイアスは今のところ見えない。11:30 前後に k≥10 で判定し `finish A "A/A instrument check"`。
- **序盤（≤20 手）の併合漏れ実測（v736、局所 60 試合・1200 手、`scratchpad/early_leak.py`）**: 同型が盤上にある手 47%、解析器が併合可能とする手 28%、併合を選んだ手 27%（可能なのに取らなかったのは 3/324 のみ）、実現併合 34%（偶発 94）。**同型があるのに併合不可が 20%**。ピース数は 0.55/手で線形増加（20 手目 11.2。人間は 5）。
- **併合不可の内訳（`twin_class.py`、次ピースの双子が存在する 568 手）**: 可能 58%、不可 42% = 露出しているが届かない 129（うち壁際 35）＋覆われている 108（うち自分の落下で 7 手以内に埋めたもの 67）。60 手まで広げると覆われ 1142 vs 露出 514、自埋め 250。
- **解析器モデルでの自埋め厳密計測（`self_bury.py`、48 試合・40 手まで 1920 手）**: 併合なし手 1416 のうち次々ピースの双子が存在 894。選んだ落下がその双子の上に着地 114（13%）→ うち次手で併合不可になったのは 37（32%）。一方、双子に乗らなかった 780 手でも次手併合不可が 529（**68%**）→ 一手の自埋めは小さい漏れ（≈0.8/試合）で、**構造的漏れは「双子がそもそも届かない配置」**（人間は平らに広げて全型を露出させている）。
- **解析器の保守性を測るプローブ v742probe（hash `512fe504863c`、`selfplay_root/strategy_v742probe.py`）**: v736 の `decide` を `_decide_base` に改名し、ラッパーで「DIRECT/NEAR の併合候補がゼロ、かつ露出（上に何も乗っていない）した同型が存在、deadline 余裕≥2.0、その真上（±2.8 clamp、候補 x との差≤0.35）の候補が非交差・非壁回転」のとき x=双子の x、reason に `OPEN_TWIN_PROBE` を付与。オフライン再生（修正済みツール）: changed 624/11155（5.6%）、DIRECT 損失 0、newcross 0、risk_top +0.26。**目的は計測**: プローブ落下の実現併合率（piece_count 減少）が高ければ解析器の併合判定が保守的すぎる＝序盤 20% の漏れの相当部分が回収可能。10:48 に局所 A/B（A=v736、B=probe、4 slots × 8、ABBA stagger、`tmp/selfplay/v742_probe`、ports 19460/18220、~11:25 完了）を起動。
- 次 tick（11:13）: A/A status（k≥10 なら finish A）、プローブ run の途中集計（`OPEN_TWIN_PROBE` 手の次手 piece_count 減少率 vs 対照の NO 手）。
...
- **オフライン再生ツールの baseline 事故（10:3x に訂正）**: `scratchpad/v737/ab737.py` の baseline `CUR` は**共有 checkout** `games/soviet_now` から import しており、checkout が他セッションの commit（strategy.py = 旧 `8fcb13b11d0c`）に切り替わっていたため「無害プローブ／同一コピーで changed 338（3.0%）」が出た。**ツールのアーティファクトではない**（08:5x 節の「3.0% を引いて読む」は撤回）。修正: REPO を `selfplay_root`（VM と analyze_board/strategy_runner/strategy/helpers 全て md5 一致を確認）にし、CUR も spec-load。再実行: 同一コピー changed 0、A/A プローブ changed 0（挙動同一を厳密確認）、v741 changed 481/11155 = 4.3%（risk_top −0.64）。共有 checkout を import する局所ツールは全て同じ事故の恐れあり。
- **リポジトリ整合**: sn-mine（`codex/no-apply-liveliness`）の strategy.py を VM 本番 v736（バイト一致を diff で確認）に戻し `tests/test_junk_consolidation.py` を削除（fixture は残置）→ commit `a0e8a9f59`、push 済み。docich 側の submodule pin は共有 checkout の他セッション commit `1aea65722`（strategy.py は旧 `8fcb13b11d0c`、私のブランチの子孫ではない）のままで、`a0e8a9f59` は pin していない（`git commit -- games/soviet_now` は index の cacheinfo ではなく checkout を拾う）。v741 本体は git 履歴 `8edcb8e34` と VM `tmp/manual_challenge/strategy_f93dbf2edf97.py` に残る。
...
- **desktop 側（`desktop-9j2it17`, Windows）**: 当初は VM から `:50021` がタイムアウトで到達不可だったため 2 位に置いた。**04:0x にユーザー側の設定作業で公開され到達可**（下の追記参照）。
- **VM 反映（実測）**: `/home/ubuntu/docich` を `main`（95029cd）へ `git pull --ff-only --no-recurse-submodules`（VM に staged で残っていた 08-23 の webui 編集は upstream `faf7d70` に含まれていたため `git stash` に退避、VM ローカル handoff.md は `handoff.md.vm-local-bak-20260827`）。`.env` に `VOICEVOX_URLS` を追記（当初 `mac,desktop,local` 順。backup `.env.bak.20260827-voicevox-chain`。04:0x に下記のとおり `desktop,mac,local` へ変更）。`docich-webui.service` 再起動、`/api/voice/endpoints` 200。`say_enqueue.sh` は `.env` を毎回読むので worker 再起動は不要。
...
- **VM CPU 再計測（外出し後、実測）**: 合計 397% → **339%**（4 コア中）。VOICEVOX 分（生涯平均 62.7%/コア、合成中 ≈1 コア）が VM から消えた。残る上位は Chrome GPU process 96%、ffmpeg 47%、Unity renderer 25%。

### 追記 2026-08-27 04:0x JST — desktop（Windows）公開完了 → 優先順を **desktop → mac → VM ローカル** へ変更（ライブ実測済み）

- **ユーザー作業**: Windows 側の coding agent へ渡す調査・設定プロンプトを作成して依頼 → 公開完了の連絡。
- **到達確認（VM から実測）**: `http://desktop-9j2it17:50021/version` `http://100.112.104.102:50021/version` とも **200 / "0.25.2"**（connect ≈0.13 s、MagicDNS 名でも IP でも同じ）。話者一覧は 3 台すべて **43 speakers / 127 styles / sha 一致**（`ffafc596c5`）＝話者 ID の食い違いなし。
- **合成速度ベンチ（同一文・speaker=3、VM から実測）**: **desktop 1.87 s** / mac 4.32 s / **VM ローカル 25.82 s**（audio_query+synthesis 合計、wav は 3 台とも 400,940 B で同一サイズ）。ユーザー希望の順序（Windows 優先）は性能的にも最適だった。
- **変更（VM）**: `.env` を `VOICEVOX_URLS=http://desktop-9j2it17:50021,http://azmacminim4:50021,http://127.0.0.1:50021` へ書き換え（backup `.env.bak.20260827-voicevox-order`）。`docich voicevox endpoints --reset` で 03:22 の切り戻し試験由来の古い失敗数を消去。webui の表示順も 1=desktop / 2=mac / 3=local に更新されたことを API で確認。worker 再起動は不要（`say_enqueue.sh` は `.env` を毎回読む）。
- **ライブ実測**: 04:01:31 に読み上げを 1 件投入 → 04:01:42 に **`active_url=http://desktop-9j2it17:50021`**（ok_count=1、last_ms=4388＝複数チャンク合計）。切替後の VM ローカル `/synthesis` は **0 件**（`journalctl -u voicevox`）。
- **フォールバックの挙動は 03:2x の試験で確認済み**: 上位が落ちれば 30s×2^n（上限 900s）のバックオフで休ませつつ次段へ自動で降りる。desktop が落ちれば mac、両方落ちれば VM ローカル。
- **未実施**: desktop の常時稼働性（スリープ・VOICEVOX 自動起動）の確認は Windows 側の報告待ち。落ちても自動フォールバックするため運用上の即時リスクは無し。
...
- **もう一種類の漏れも修正済み（ユーザー選択 B、commit `1b5d9da18`）**: 00:51:23 の返答
  （`spoken_history/20260827_005123_14629_main.txt`）は `<think>` を使わない**日本語の作業メモ**が
  漏れていた（「インジケーターのスクリプトは…実行できないようですが」／「以下、2件のコメント返しです。」／`---`／本文）。
  共通ガードの `WORK_NOTE_RE` を広げるとラジオにも影響するため、**コメント側だけで判定する案 B** を採用。
  `_comment_strip_worknote_head()` を追加し `_comment_guard_model_text` から通す。落とすのは
  (1) 罫線だけの行がある (2) その後ろに本文が残る (3) 前置きが作業メモ語彙にマッチする
  (4) 前置きが視聴者への呼びかけ「〜さん、」を含まない (5) 前置きが 600 字以内、を**全て満たすときだけ**。
  罫線だけの行は本文中からも落とす。`===SING===` は `=` 区切りなので罫線判定に当たらない。
  **誤爆検証: VM の直近 COMMENT 出力 40 本へ適用し、変化したのは漏れた 1 本のみ（誤爆 0）**。
  テストは 13 → 26 assertion。`lib/model_output_guard.py` は無変更（ラジオ経路に影響なし）。
  VM 反映は SHA256 一致、chat 01:17:48 / kick 01:17:54 / youtube 01:18:01 `reload complete`。
- **ライブ実測 (03:5x, 完了)**: 修正 (chat 01:17:48 reload) 以降に生成された読み上げ本文
  **4 本すべてで漏れゼロ**（`spoken_history` の 01:37 / 02:18 x2 / 03:03）。
  検査は `</think>` / `analysis` / `tool_call` / `Let me` / `I need to` / `Analyzing` /
  サンドボックス / ネットワーク制約 / 「以下、N件のコメント返し」/ 罫線だけの行 /
  WebFetch / exec_command の全文グレップ。同じパターンを既知の漏れ 2 本へ当てると
  両方 DETECTED、クリーンな返答は clean になることも確認済み（＝沈黙が「検知漏れ」でないことの裏取り）。
- **常駐監視**: 読み上げ本文へのメタ出力漏れ再発検知ウォッチャーを起動
  （`spoken_history` の新規ファイルを 30 秒ごとに上記パターンで検査、漏れた時だけ通知）。
  「今なにしてる」系コメントの監視、実レイド監視と合わせて 3 本稼働中（セッション内のみ）。
...
- **ライブ実測 (23:21:54, 完了)**: 実際の視聴者コメントで生成された
  `tmp/debug/ai_dispatch/20260826_232154_COMMENT_opencode-go_muse-spark-1.2-contributor_prompt.txt`
  （12,141 バイト、chitchat カテゴリ）の **37 行目に `【Current Ops / Behind-the-Scenes Memo】` が
  ちょうど 1 回**入り、バナー・改善待機中・A/B 91 試合・handoff 3 件が展開されていることを確認。
  未置換の `${comment_ops_context}` は直近 4 本すべてで 0 件。
  この回は `_broadcast_host_mode` が `soren91` を返しており、メモも
  「いまメイン画面はメリケンAIのソ連ゲーム91(対戦版)」と正しく切り替わっていた。
- **未確認 / 次にやること**:
  - 「今なにしてるの」系のコメントへ、このメモを使った返答が実際に出るかの実測（該当コメント待ち）。
...
## 2026-08-27 03:3x JST — v741 JUNK_CONSOLIDATION（序盤ジャンクの隅寄せ）を実装・自己対戦 A/B 中

- **設計（fable、序盤の盤面形成方針の比較）**: 型順レーン（v731/v732/v736 と衝突、~20 手/試合変更）・二領域方針・nextNext 予約（v739 と同類）を退け、**ジャンク集約**を採用。根拠: 人間は序盤の T1–3 を右壁（x≈+2.7、5/5）に置き 40 手時点 9 駒（自動 22）、自動の T1–3 の置き場は x 平均 0・sd 1.15 で無秩序（|x|≥2 は 13%）、壁際の序盤ジャンクは転がり 7%（中央 32%）・5 手後の埋没 43%（60%）。両壁が高型で塞がれる手が 39〜84% あるため 2 段構え（隅 → 開いたジャンク塊）。
- **実装（soviet_now `8edcb8e34`、hash `f93dbf2edf97`、未投入）**: T1–3・NO 盤面・DIRECT/NEAR 無し・margin≥2.0・非 crossing・壁際回転リスク無し・被覆タグ無し・ロシア不在のとき、tier 1 = 床か T≤4 の上にしか落ちない側の隅（±2.8、ジャンクの多い側優先）へ +350×(1−|dx|)（|dx|≤1.0）、tier 2 = 開いたジャンク塊（|dx|≤1.0 の T≤4 が最多、上端≤1.0、目標は ±2.8 に丸め）。fable 案からの追加: `wall_rotation_risk` 候補を除外（塊が壁際にしか無い局面で x=−3.0 を選んでいた）。オフライン（57 試合 5,169 手）: 変更 4.6%（4.2 手/試合、駒数<10 で 14%）、DIRECT 喪失 0、新規交差 0、risk_top −0.51、新規壁リスク 0。テスト 8 本 + fixture 6 件（隅 3・塊 2・壁リスク否定 1）、全体テストの失敗集合は HEAD と同一。`tools/selfplay_ab.py` の stagger 式と idx（スロット内連番→ブロックがスロット内 ABBA）を修正。
- **自己対戦 A/B（v736 vs v741、4 スロット、ABBA stagger）**: 第 1 バッチは 24 試合（各腕 12）で外部停止（セッション休止に伴いバックグラウンドジョブが落ちる）: B 1933 vs A 1642（ブロック差 +556、SE 488、p=0.5）、手数 103 vs 96、併合/手 0.547 vs 0.531、20 手時点 10.6 vs 11.2、40 手時点 20.3 vs 19.1、カザフ 6 vs 5 — 雑音圏。第 2 バッチ（4×24 = 96 試合）を **nohup サブシェルで分離起動**（03:31、`selfplay_root/tmp/selfplay/v741_ab2`、ログ `tmp/v741_ab2.log`）。事前登録: 主指標 40 手時点の駒数（目標 −2.0、MDE 2.0@24 ブロック）、副 = raw score・併合/手・手数・T14/T15、look k=12/24、害停止 UCB90(score)<−300、無益 k≥12 で駒数削減の UCB90<1.0。実戦 A/B へ進むのは駒数 ≤−2.0 かつ p<0.025 かつ raw ≥ +150（UCB90 ≥ +300）のとき。
- **自己対戦 A/B 中間（04:30、第 2 バッチ 65/96 試合、第 1 バッチと合算 89 試合、tainted 0）**: 合算 raw **A 1458 (n44) vs B 1864 (n45)、B−A +406（SE 159、z +2.6）**、eval +2271（z 2.7）、手数 +10.5（z 2.3）、併合/手 +0.051（z 2.3）、カザフ 13 vs 19、ロシア 1 vs 3。第 2 バッチのブロック解析（スロット内 ABBA、k=15）: raw +507（SE 200、**符号反転 p=0.020**）、eval +2676（p=0.016）。**主指標の 40 手時点の駒数は −0.4（n.s.）で動いていない**（20 手時点は +0.5 n.s.）＝効果は序盤の駒数ではなく中盤以降の併合・生存で出ている（隅のジャンクが後で併合されやすい？要検証）。実物理での +30% は配信 A/B（74/腕）で確認可能な大きさ。v741 を VM に `tmp/manual_challenge/strategy_f93dbf2edf97.py` としてステージ済み。**バッチ完了（残り 31 試合）を見てから `tools/ab_ctl.sh start … ABBA` で実戦 A/B**（改善ループの候補と枠が競合しないよう、gate は dry-run のまま）。
- **自己対戦 A/B 最終（第 2 バッチ 96 試合、各腕 48、tainted 0、03:31–05:2x）**: ブロック解析（スロット内 ABBA、k=24）raw **B−A +319（SE 151、符号反転 p=0.043）**、eval +1732（p=0.034）、手数 97.8 vs 87.8、併合/手 0.561 vs 0.510、カザフ 19 vs 12、ロシア 3 vs 1。合算 120 試合: raw +313（z 2.4）、手数 +9.6（z 2.5）、併合/手 +0.043（z 2.3）、40 手時点の駒数 −0.5（n.s.）。事前登録の主指標（駒数@40 ≤ −2）は未達だが、実スコア・生存・併合・到達が両バッチで一貫して改善（+21%）しており、+300 級は配信 A/B で確認可能 → **実戦 A/B へ**。
- **実戦 A/B 経過（06:30、各腕 7、k=3）**: raw A 1648（sd 966、eval 21321 の大試合を含む）vs B 962（sd 262）、mean(B−A) = −871（SE 305、UCB90 −480）、手数 92 vs 75、併合/手 0.494 vs 0.428、20/40 手時点の駒数 B 11/21 vs A 12/25（B の序盤は軽い）、カザフ 1 vs 0。**自己対戦（+319）と逆方向**。k=6（~07:00）で UCB90<0 なら害停止 → finish A。自己対戦との乖離の候補: 雑音（n=7）、配信環境の差（局所 SD 879 vs 配信 650、負荷、settle 実測）。
- **実戦 A/B 開始（05:30:38）**: `tools/ab_ctl.sh start tmp/manual_challenge/strategy_f93dbf2edf97.py ABBA`。A=v736 253cc67e0c1b（root）、B=v741 f93dbf2edf97、revert 先 = v736、REGRESSION_DISABLED=1、gate は dry-run のまま（改善ループの候補は待機）。事前登録: 主指標 raw score（74/腕で +300 を検出、MDE(74)=364@sd 879 相当だが配信 sd 650–800）、副 = 併合/手・手数・T14/T15・20/40 手時点の駒数；`ab_decide` の害停止（k≥6, UCB90<0）・無益停止（k≥12, UCB90<150）・採用 look k=19/37。判定は自動ではなく私が各 tick で `tools/ab_ctl.sh status` と `ab_decide.py` を見て `finish A|B`。
- **改善ループ**: 2 回目のジョブが 03:09 から解析フェーズで長時間実行中（`analyze_retry1`）。候補は root に触れない。
- **運用メモ**: ツールのバックグラウンドジョブはセッション休止で落ちる（3 度目）。長時間の自己対戦は `(nohup … &)` で分離し、各 tick でログを読む。macOS に `setsid` は無い。
...
- **A/A 較正（ローカル自己対戦、v736 vs v736、4 スロット×8 = 32 試合、00:31–01:02、エラー 0・tainted 0）**: 1 試合平均 212 s、4 スロットで ~61 試合/時（6 スロットで ~90/時見込み）。局所 SD（試合単位）: raw score **879**（配信 650–800）、eval 3825、手数 26、併合/手（連鎖込み）0.113、20 手時点の駒数 2.37、**40 手時点の駒数 4.44**。腕対称性: A 1458 vs B 1694（ブロック差 p=0.62、B に T15 1 試合）＝雑音内。**MDE（150/腕、80%）**: raw 284、併合/手 0.036、駒数@20 0.77、**駒数@40 1.44**（人間との差は 13 駒）→ 序盤の盤面形成方針は 40 手時点の駒数で最も感度よく検出できる（2×50 試合 ≈ 1 時間でも MDE ≈ 2.5 駒）。記録 `selfplay_root/tmp/selfplay/aa_calib/`。
...
- **loop（22:3x）**: A/B 83 試合（A 41 / B 42、k=20）: raw A 1377 vs B 1415、**mean(B−A) = +39（SE 117、UCB90 +189）→ CONTINUE**（k=19 の look は採用なし、無益停止 150 も未達）、併合/手 0.492 vs 0.491、手数 86.5 vs 82.0、カザフ B 10 / A 8、ロシア 0/0。中立。最終 k=37（~05:30）まで継続（途中 UCB90<150 で無益停止）。
- **改善ループ初候補（dry-run、22:05 出力、hash `af24f8469e58`）**: 48 試合蓄積で改善ジョブが 21:36 起動 → 候補は root に適用されず `tmp/state/ab_candidate/` に出力（`candidate_ready` として回収、lock 解放、蓄積リセット）— **ゲート配線は設計どおり動作**。内容は数値定数 ~36 箇所のランダム摂動（wildcard 型、差分 26 行、helper 変更なし）。オフライン A/B（57 試合 5,169 手）: 変更 8.0%、併合喪失 0、新規交差 0、例外 0、risk_top −0.041 ＝ 害なし・根拠なし → **方針どおり実戦枠には回さず見送り**（`tmp/history/ab_candidate_skipped_offline_*` に退避）。次候補は 48 試合後（~8 時間）。
- **並行プロジェクト（ユーザー通知 22:4x）**: soren91（メリケンAI）再稼働プロジェクトが別セッションで同時並行稼働中。同一 VM・同一 checkout・同一 .env を共有するため、(1) soren91 系のファイル/プロセス/.env キー（`SOREN91_*`, `MERIKEN_*`, `manual_meriken_mode`）には触れない、(2) 境界 pause のマーカー使用前に soren91 側の利用を確認、(3) 負荷・試合間隔・スコア漂流を戦略のせいにする前に soren91 側の稼働と突き合わせる、(4) checkout の未コミット変更・リモート先行コミットは他セッション由来としてパス指定コミットのみ。帰属はインターリーブ A/B に依拠。
