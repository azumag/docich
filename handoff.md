...
> 直前セッション: 併合/手を主要指標に切替（容量モデル `手数 = 42/(1−併合/手)`、ソ連建国には 0.83 必要、現 root v748 = 0.559）。**実戦 v752 vs v748 は k=22 で score +122（SE 151）、併合/手 +0.008（z 0.42）＝ほぼ中立**（idx=0 は A/B 開始前に始まった v736 の試合なので判定時に除外）。連鎖併合の選択は既に 98% 最適（伸びしろ無し）。高併合試合の特徴は「25 手目が平ら（最高点 −1.90 vs −1.51）・露出率 0.397 vs 0.321」→ **v754 EARLY_FLATNESS（hash 9a1013eedf1e、v748 ＋ 駒数≤26 の非併合手で高さペナルティ ×2.5）** を作成、局所 P/Q（`v754_p`/`v754_q`、06:37–~08:00）で機構検証中。実戦 A/B の順番: v752（k=50 は本日 15:00 頃）→ v754。
