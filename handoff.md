...
> 直前セッション: **局所自己対戦は「併合/手」でも候補を順位付けできないと確定**（v741 +0.043 / v744 +0.042 / v748 +0.055 と全て局所は正、実戦は −528 / −39 / **+194**）。→ **局所は機構検証（タグ発火・盤面指標が意図どおり動くか）専用、採否は実戦 A/B のみ**。実戦 A/B（A=v748 / B=v752）は k=25 で score +192（SE 150）、併合/手 +0.012（z 0.67）、T15 2 vs 1 → 継続（k=50 判定は本日 15:00 頃、idx=0 の v736 汚染 1 件を除外）。v754 EARLY_FLATNESS は**機構が逆方向**（25 手目の最高点 −1.83 vs v748 −1.99 ＝ 平らにならず、露出率も低下）で効果 ≈0 → 棚上げ。次の実戦候補は未定（v752 の結果を見て決める）。
