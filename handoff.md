...
> 直前セッション: 壁反射修正 mode 1 は n=6 で判定待ち（≥12）。人間との条件付き併合率は同等（開いた相方あり 59.5% vs 60.0%）で戦術差は尽きた。ソ連建国は幾何的に成立、制約は材料と ~250 手の生存（併合/手 0.51 vs 人間 0.63、維持には ~0.8+）。v737(c) は見送り継続。SOREN_SETTLE_REQUIRED=3 維持（mode 1 判定後に 4 の実験候補）。
