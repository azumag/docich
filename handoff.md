...
> 直前セッション: **v748 SEAT_LANES（897803192d9f）の実戦 A/B を 03:30 に開始**（A=v736 253cc67e0c1b / B=v748、ABBA、REGRESSION_DISABLED=1、VM `tmp/manual_challenge/strategy_897803192d9f.py`、validator OK、バナー「v748 実戦 A/B」）。事前登録は従来どおり（k≥6 で UCB90<0 → 害停止 `finish A`、k≥12 で UCB90<150 → 無益停止、looks 19/37 で `ab_decide` ADOPT → `finish B`＋repo commit）。局所根拠: 同時 P/Q 2 run・4 instance プール（60/60）で **raw +270（SE 146、z 1.84）、ブロック効果 ≈ +231**、手数 101.5 vs 94.8、20 手目ピース 10.3 vs 11.5、40 手目 18.4 vs 21.2（v741–v747 で唯一、盤面圧縮の機構指標が一貫して動く）。P 側（B=v748）が常に大きく正（+576/+802）、Q 側（A=v748）が負（−288/−167）という偏りあり。v744 実戦は 02:30 に REJECT_FUTILE（−39）で finish A 済み。
