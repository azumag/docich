...
> 直前セッション: v736（hash 253cc67e0c1b）初戦でロシア建国（143 手）、n=8 で判定待ち（n≥12）。v737(c)（cover/beside 別ランプ）は corpus で +0.00207 併合/手の限界的 go、埋没コストは実測で無し（v738 不要）。v736 判定後に v737 投入可否を決める。SOREN_SETTLE_REQUIRED=3 維持、判定窓では SOREN_SETTLE_* を変更しない。
