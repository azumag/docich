...
> 直前セッション: 壁反射修正 mode 1 は n=16 で中立 → 維持。静止待ち SOREN_SETTLE_REQUIRED=4 を 04:30 に単独実験開始（04:33 の試合から実測、判定 n≥24 で 3 の窓 平均≈1640 / 併合/手 0.34 と比較、窓中は decide/analyzer を変更しない）。人間との差は序盤 40 手の階段配置（t+1 型の隣に置く: 50% vs 29%）と特定、v738 STAIRCASE_ADJ を設計中（投入は判定後）。
