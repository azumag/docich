...
> 直前セッション: **ソ連建国までの距離を定量化（710 試合）**: 試合終了は「盤面容量 C≈42 個（sd 4.1、戦略で動かせない）」に達した時点で、`手数 = C/(1 − 併合/手)`。実測 710 試合で 200 手以上は 0（最長 199）。**ソ連建国に必要な 250 手には 併合/手 ≈ 0.83 が必要**、現 root v748 は 0.559（v736 0.521、v748 の改善幅は +0.038）→ **v748 級の改善があと約 7 回必要**。副産物: **併合/手 は score より統計効率が 2 倍（同一データで z 2.82 vs 1.97、CV 20% vs 43%）** → 今後の A/B は「併合/手」を主要指標にし、必要 n を半減（効果 +0.03 なら各腕 183、+0.05 なら 66）。実行中の v752 vs v748 長期 A/B（23:33 開始、k=18 で score +106）は事前登録どおり score で判定し、併合/手 は副次指標として併記する。root は v748 `897803192d9f`、リポジトリ sn-mine `cfd651b28`。
