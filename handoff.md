...
> 直前セッション: v741（序盤ジャンク隅寄せ、hash f93dbf2edf97）はローカル自己対戦で B−A +406 raw（z 2.6、ブロック p=0.02、89 試合）と有望。40 手時点の駒数は不変で効果は中盤以降。バッチ完了後に `tools/ab_ctl.sh start tmp/manual_challenge/strategy_f93dbf2edf97.py ABBA` で実戦 A/B（74/腕）へ。本番 v736、改善ループ dry-run（2 回目ジョブ実行中）。
