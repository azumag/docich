...
> 直前セッション: v744 実戦 A/B は k=23（各 47）で +43（SE 130、UCB90 +210）→ 継続（look 37 ~05:00、実戦効果 ≈0 見込み）。v747（終盤併合優先）は局所再現 P2 −425 / Q2 +67 → 合成 −179、初回 +174 とプールで **≈0**（raw 1783 vs 1811）→ 単独では棚上げ（機構は正しいが発生 0.36 回/試合で測定不能）。**VM のランナー差し替えは不要になった**（v747 を実戦に出す場合のみ必要。分類器ブロックの件は保留のまま）。局所調整 5 候補（v741–v747）が全て ±200 以内で、次は全体方針 **v748 SEAT_LANES（v744 ＋ T1–T8 の型別定位置: 高型側 S の反対壁から T1/T2 2.8, T3 2.4, T4 2.0, T5 1.55, T6 1.1, T7 0.6, T8 0.15、非併合手に ±220 の距離加点、hash 897803192d9f）**。オフライン changed 33.8%・DIRECT 損失 0・newcross 0・risk −0.14。局所同時 P/Q `v748_p`/`v748_q` を 01:35 起動（~03:00）。
