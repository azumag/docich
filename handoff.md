...
- **worker復旧**: 古いradio生成子シェル43件をユーザー承認後に終了。pause marker解除後、radio worker PID `528038` が supervisor PID `856212` 直下で18:42起動。`soren-runtime.service=active`、stream 1系統、ゲームrunner継続。**次回の実読み上げは未確認**（18:42開始の時事ニュース調査が先行中）。選定経路・候補鮮度は確認済みだが、実際の音声で新記事が流れるまでは視聴者可視の完了とはしない。
