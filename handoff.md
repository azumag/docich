...
> 直前セッション: 実戦 root = **v752 `a557db55896b`**。解析器 A/B（両腕 v752、B のみ `ANALYZE_BOARD_LANDING_ARC=3`）は k=18 で score −219（UCB90 +63、害停止せず）・併合/手 −0.027（CI90 [−0.055, +0.002]）と 0 に回帰中、規則どおり k=50（~06:00）まで継続。劣化機構は判明済み（締切余裕が広く見える → 自作評価軸の安全ゲートが開いて発火 +4pt → 悪化。**軸は最適点付近かやや過剰**）。**本 tick の重要な否定結果: settled した同型ペアは「寄せ」で併合できない**（近接ペア 5573 件の 3 手以内併合は 3%、ドロップ位置に無関係）。**終了時の駒の 96% は同型の相方が盤上にいるのに併合していない**（単独はわずか 1.6/試合）。埋没は多くが一時的（解放中央値 2 手、27% は埋没のまま併合）＝「覆わない」系の軸が中立だった理由。**唯一の実効機構は「落とすピースが露出した同型に届くか」**（露出している手は 38%、その手の併合率 **76%**）。次候補 v756（`4b618d728ca7`、v752＋EXPOSURE_KEEP）は VM 待機中だが、上記より優先度は再考の余地あり。
