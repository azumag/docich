...
> 直前セッション: **v744 実戦 A/B は k=27（55/54）で REJECT_FUTILE（mean −39、UCB90 122<150）→ 02:30 `finish A`、root v736（253cc67e0c1b）、REGRESSION_DISABLED=0、記録 `tmp/history/ab_20260828_023015_*`（110 件、迷子 idx 109 統合済み）、バナー停止。** v744 は局所 +190／実戦 ≈−39（v741 と同型の乖離、v744 は害ではない）。**v748 SEAT_LANES（897803192d9f）** の局所同時 P/Q 初回（各 30）: P +576（SE 378）/ Q −288（SE 213）→ 合成 ≈ +144（SE ≈217、腕文字 +432）で判定不能だが、**機構指標は狙いどおり**（20 手目ピース 10.0 vs 11.5、40 手目 18.9 vs 21.6、手数 99.2 vs 92.9、raw プール 1840 vs 1661）。再現 `v748_p2`/`v748_q2`（02:35–~04:00）実行中。実戦 A/B は無し（次は v748 が局所プールで ≥+250 なら開始）。
