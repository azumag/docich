...
> 直前セッション: **解析器の着地予測に系統誤差を発見: `get_landing_info` は「横に重なれば相手の真上に乗る」箱積みモデルで、実測 852 手で着地 y を平均 +0.69 高く予測（95% が過大、駒の上着地 n=834 で +0.72）**。これが終盤の併合拒否（超過フラグの過剰）と併合見落とし（併合予測の見落とし 133/1242 = 10.7%）の共通の根本原因と考えられる。第一次修正（円弧接触モデル、env `ANALYZE_BOARD_LANDING_ARC`、既定 0 で本番不変、`selfplay_root/analyze_board.py`、原本 `.pre_arc`）は**着地誤差を +0.69 → +0.49 に改善したが併合予測の正答率は 88.8% → 81.4% に悪化**（見落とし 133 → 226）＝着地高さだけ下げても接触/併合判定が箱積み前提のままなので対で直す必要がある。次: 併合判定側（`merge_grade` の接触テスト）を円弧接触に合わせて同時修正し、着地誤差と併合予測の両方が改善する版を作る。実戦 A/B（A=v748 / B=v752）は各 72 で 併合/手 +0.009（z 0.61）、score +104、T15 3 vs 2 → 15:00 頃に k=50 判定。
