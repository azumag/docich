...
- 更新したら `games/soviet_now/tools/build_ops_brief.sh` を実行して
  `games/soviet_now/prompts/ops_brief.md`（最新セクション見出し 3 件）を作り直し、VM へも配る。
  この 3 行はコメント返しプロンプトの【いまの配信・運用状況メモ】として視聴者への返答に使われる。
  VM の `handoff.md` は古いまま残っているので、生成は必ず docich チェックアウト側で行う。
...
- **自動文面**: 最初の `start` では、タイトルを定型文へ差し込まず、**本文をそのまま独立した自然な一文として読む**。本文は `VMで作業中音声の文面を実測しています` のように、それだけで意味が通る、簡潔なです・ます調で書く。`現在、〜の作業を進めています`、`ただいま〜を進めています`、`詳細は「〜」です` などの枠文を機械的に足さない。本文が空の場合だけタイトルを読む。240字で丸め、`lib/outbound_queue.sh` の `enqueue_audio_text` へ `work_indicator` として渡す。
- **粒度（大くくり）**: バナーはフェーズごとに更新するが、**音声は作業セッションの最初の `start` だけ**にする。同じセッション中の `解析中 → 実装中 → 検証中 → デプロイ中` はバナーだけ更新し、追加で読まない。短い別作業が続く場合も開始音声は15分に1回まで。`stop` の完了音声は、開始音声を流し、かつ3分以上続いたセッションだけにする。重複 `stop` は読まない。判定状態は `tmp/state/work_audio_last.json` に保存する。
- **手動（直接 enqueue したい場合）**: `ssh -i ~/.ssh/id_rsa ubuntu@129.146.54.105 "cd /home/ubuntu/soren && source lib/outbound_queue.sh && enqueue_audio_text \"VMで作業中音声の文面を実測しています。\" work_indicator"`。ローカルの `audio_worker` が動いていなくても VM 側の `audio_worker` が再生する。手動でも単独で自然に読める簡潔な文にし、過度にへりくだった表現や定型の前置きは避ける。
