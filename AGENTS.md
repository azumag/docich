...
- 例外（pacman4console のみ）: maze は正方形タイル前提のアートだが端末セルは 1:2 のため、そのままでは縦に約2倍で表示される。`config/games/pacman4console.toml` の `[cli] cell_aspect = "1:2"` による正方形セル補正（配信側の水平2倍）だけを許可する（2026-09-22 オーナー承認）。ゲーム本体の窓・内部解像度・端末寸法は変更しない。他のゲームへ広げない。
